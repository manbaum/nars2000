...
//  $GetNextArbToken
//
//  Return the next value from a token as a ARB.
//***************************************************************************

APLARB GetNextArbToken
    (LPTOKEN  lptkArg,                      // Ptr to arg token
     APLINT   uIndex,                       // Index
     APLSTYPE aplTypeArg,                   // Arg storage type
     LPUBOOL  lpbRet)                       // Ptr to TRUE iff the result is valid

{
    ...
    *lpbRet = TRUE;
    ...
    // Attempt to convert the data to ARB
    (*aTypeActConvert[aplTypeArg][ARRAY_ARB]) (lpMemArg, uIndex, &atRes, lpbRet);
