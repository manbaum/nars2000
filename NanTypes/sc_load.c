...

{
    ...
    mp_prec_t         uCommPrec = 0;        // Common precision for array of VFP/ARB numbers
...
////////////////////////sscanfW (lpwszProf, SCANFSTR_APLINT, ((LPAPLINT) lpMemObj)++);
                ...
                case ARRAY_ARB:
                    // Parse the sequence of ARB numbers
                    ParseBAxFs (&(LPAPLARB) lpMemObj,   // Ptr to ptr to output save area
                                 aplNELMObj,            // NELM of the input
                                &lpwszProf,             // Ptr to ptr to input
                                 lpwszFormat,           // Ptr to scratch area
                                 lpMemPTD,              // Ptr to PerTabData global memory
                ...
                case ARRAY_HC2I:
...
#ifdef PERFMONON
                    {
                        ...
                        MySprintfW (wszTemp,
                                    sizeof (wszTemp),
                                   L"End of HC2I on %I64u elements",
                                    aplNELMObj);
                        PERFMONSHOW (wszTemp)
                    }
#endif
                ...
                case ARRAY_HC4I:
...
#ifdef PERFMONON
                    {
                        ...
                        MySprintfW (wszTemp,
                                    sizeof (wszTemp),
                                   L"End of HC4I on %I64u elements",
                                    aplNELMObj);
                        PERFMONSHOW (wszTemp)
                    }
#endif
                ...
                case ARRAY_HC8I:
...
#ifdef PERFMONON
                    {
                        ...
                        MySprintfW (wszTemp,
                                    sizeof (wszTemp),
                                   L"End of HC8I on %I64u elements",
                                    aplNELMObj);
                        PERFMONSHOW (wszTemp)
                    }
#endif
                ...
                case ARRAY_BA2F:
                    // Parse the sequence of BA2F numbers
                    ParseBAxFs (&(LPAPLARB) lpMemObj,   // Ptr to ptr to output save area
                                 aplNELMObj,            // NELM of the input
                                &lpwszProf,             // Ptr to ptr to input
                                 lpwszFormat,           // Ptr to scratch area
                ...
                case ARRAY_BA4F:
                    // Parse the sequence of BA4F numbers
                    ParseBAxFs (&(LPAPLARB) lpMemObj,   // Ptr to ptr to output save area
                                 aplNELMObj,            // NELM of the input
                                &lpwszProf,             // Ptr to ptr to input
                                 lpwszFormat,           // Ptr to scratch area
                ...
                case ARRAY_BA8F:
                    // Parse the sequence of BA8F numbers
                    ParseBAxFs (&(LPAPLARB) lpMemObj,   // Ptr to ptr to output save area
                                 aplNELMObj,            // NELM of the input
                                &lpwszProf,             // Ptr to ptr to input
                                 lpwszFormat,           // Ptr to scratch area
...
//  $ParseBAxFs
//
//  Parse a sequence of BAxFs
//***************************************************************************

void ParseBAxFs
    (LPAPLARB    *lplpMemObj,   // Ptr to ptr to output save area
     APLNELM      aplNELMObj,   // NELM of the input
     LPWCHAR     *lplpwszProf,  // Ptr to ptr to input
     LPWCHAR      lpwszFormat,  // Ptr to scratch area
     LPPERTABDATA lpMemPTD,     // Ptr to PerTabData global memory
...
{
    mp_prec_t uDefPrec,         // Default precision to use when inputting VFP/ARB numbers
              uOldPrec;         // Old precision to be restored later
    LPWCHAR   lpwCharEnd;       // Temporary ptr
    LPCHAR    lpFmt;            // Temporary ptr
    APLU3264  uLen;             // Temporary length
    ...
    // Get the current precision
    uOldPrec = mpfr_get_default_prec ();    // Use MPFR here

    // If we're using the default precision, ...
    if (uCommPrec EQ 0)
        // Use it
        uCommPrec = uOldPrec;
    ...
    for (uObj = 0; uObj < aplNELMObj; uObj++)
    {
        ...
        if (**lplpwszProf EQ L'(')
        {
            ...
            // Set the default precision
            mpfr_set_default_prec (uDefPrec);   // Use MPFR here

            // Skip past the trailing paren
            (*lplpwszProf) = SkipPastCharW (*lplpwszProf, L')');
        } else
            // Set the default precision
            mpfr_set_default_prec ((uCommPrec EQ 0) ? uOldPrec : uCommPrec);    // Use MPFR here
        ...
        *lpwCharEnd = wcTmp;
        ...
        // Initialize the save area to 0/0 using the precision set above
        Myarb_init (*lplpMemObj);

        // Check for positive infinity
        if (strncmp (lpFmt, TEXT_INFINITY, strcountof ("-" TEXT_INFINITY)) EQ 0)
        {
            arb_set_inf ((*lplpMemObj)++, 1);
        ...
        {
            // Convert the string to ARB
            // Note this routine converts both Midpoint and Radius if present
            arb_set_str2 ((*lplpMemObj)++, lpFmt, uCommPrec);

            // Count the scanned chars including the deparator and Radius
            uLen = strspn (lpFmt, ARB_CHARS);
        } // End IF/ELSE/...
        ...
        for (uCnt = 0; uCnt < hcXSepLen; uCnt++)
        {
            // Initialize the save area to 0/0
            Myarb_init (*lplpMemObj);
    ...
    // Restore the default precision
    mpfr_set_default_prec (uOldPrec);   // Use MPFR here
} // End ParseBAxFs
