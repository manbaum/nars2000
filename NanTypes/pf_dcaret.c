...
PRIMSPEC PrimSpecDownCaret = {
    ...
    NULL,   // &PrimFnMonDownCaretBA1FisBA1F,
    NULL,   // &PrimFnMonDownCaretBA2FisBA2F,
    NULL,   // &PrimFnMonDownCaretBA4FisBA4F,
    NULL,   // &PrimFnMonDownCaretBA8FisBA8F,
    NULL,   // &PrimFnMonDownCaretARBisBA1F,
    NULL,   // &PrimFnMonDownCaretARBisBA2F,
    NULL,   // &PrimFnMonDownCaretARBisBA4F,
    NULL,   // &PrimFnMonDownCaretARBisBA8F,
    ...
    NULL,   // &PrimFnDydDownCaretBisBA1FvBA1F, -- Can't happen w/DownCaret
    NULL,   // &PrimFnDydDownCaretBisBA2FvBA2F, -- Can't happen w/DownCaret
    NULL,   // &PrimFnDydDownCaretBisBA4FvBA4F, -- Can't happen w/DownCaret
    NULL,   // &PrimFnDydDownCaretBisBA8FvBA8F, -- Can't happen w/DownCaret
    ...
    &PrimFnDydDownCaretBA1FisBA1FvBA1F,
    &PrimFnDydDownCaretBA2FisBA2FvBA2F,
    &PrimFnDydDownCaretBA4FisBA4FvBA4F,
    NULL,   // &PrimFnDydDownCaretBA8FisBA8FvBA8F, -- Can't happen w/DownCaret
...

{
    ...
    __try
    {
        ...
        FltOut (L"aplLft.parts[0] == ", &aplLft.parts[0]);
        FltOut (L"aplLft.parts[1] == ", &aplLft.parts[1]);
        FltOut (L"aplLft.parts[2] == ", &aplLft.parts[2]);
        FltOut (L"aplLft.parts[3] == ", &aplLft.parts[3]);
        DbgMsgW (WS_CRLF);
...

{
    ...
    __try
    {
        ...
        {
#if (defined DEBUG) && (defined DEBUG_FMT)
        RatOut (L"aplLft.parts[0] == ", &aplLft.parts[0]);
        RatOut (L"aplLft.parts[1] == ", &aplLft.parts[1]);
        RatOut (L"aplLft.parts[2] == ", &aplLft.parts[2]);
        RatOut (L"aplLft.parts[3] == ", &aplLft.parts[3]);
        DbgMsgW (WS_CRLF);
...
//  $GcdBA1F
//
//  GCD (Greatest Common Divisor) for aplArbs
//***************************************************************************
...

{
    ...
    // Ensure both arguments are non-negative
    arb_abs2 (&aplLft, &aplVfpLft);
    arb_abs2 (&aplRht, &aplVfpRht);

    while (arb_cmp_d (&aplLft, GCD_CT) >= 0)
    {
////////aplTmp = aplLft;
////////aplLft = aplRht % aplLft;
////////aplRht = aplTmp;
        ...
        if (guTest > 0)
        {
            ArbOut (L"aplLft = ", &aplLft);
            ArbOut (L"aplRht = ", &aplRht);
        } // End IF
#endif

        arb_set (&aplTmp, &aplLft);
        arb_mod (&aplLft, &aplRht, &aplLft);
        arb_set (&aplRht, &aplTmp);
    ...
    // The sign of the result is non-negative
    arb_abs2 (&aplRht, &aplRht);

    // We no longer need this storage
////arb_clear (&aplRht);        // Returned as result
    Myarb_clear (&aplLft);
    Myarb_clear (&aplTmp);
...
//  $GcdBA2F
//
//  GCD (Greatest Common Divisor) for BA2F
//***************************************************************************
...

{
    ...
    APLFLOAT fQuadCTOld,                // Original []CT
             fQuadCTNew = 1E-10,        // Default  []CT for comparisons and L|R
             fQuadCTvL  = 4E-15;        // Default  []CT for comparisons and L|R
    APLARB   aplMagLft;                 // Temps for magnitude
    int      iRet;                      // Comparison result
    ...
    // Copy to temp vars
    arb2f_init_set (&aplLft, &aplBA2FLft);
    arb2f_init_set (&aplRht, &aplBA2FRht);
    ...
    __try
    {
        ...
        {
            // T = L
            arb2f_set (&aplTmp, &aplLft);

            // L = L | R
            aplMod = ModBA2F (aplLft, aplRht);
            arb2f_set (&aplLft, &aplMod);
            arb2f_clear (&aplMod);

            // R = T
            arb2f_set (&aplRht, &aplTmp);

            // Calculate the magnitude
            aplMagLft = MagBA2F (aplLft);

            // Compare the magnitude with a small number
            iRet = arb_cmp_d (&aplMagLft, fQuadCTvL);

            // We no longer need this temp
            arb1f_clear (&aplMagLft);
        } while (iRet > 0);
        ...
        // Loop through all of the parts of all of the result looking for negative values
        for (uIndex = 0, i = 2 - 1; i >= 0; i--)
            uIndex = (3 * uIndex) + 1 + arb_sign (&aplRht.parts[i]);
        // Get the table index
        uRht = gGcdRotateTab2[uIndex];

        // Make the righthand unit
        MakeUnit (ARRAY_BA2F, uRht, &atRht);

        // Multiply on the right by the righthand unit
        aplMul = MulBA2F_RE (aplRht, atRht.aplBA2F);

        // Save in the result
        arb2f_set (&aplRht, &aplMul);

        // We no longer need this resource
        arb2f_clear (&atRht.aplBA2F);
        arb2f_clear (&aplMul       );
        arb2f_clear (&aplMod       );
        arb2f_clear (&aplTmp       );
////////arb2f_clear (&aplRht       );       // Returned as result
        arb2f_clear (&aplLft       );
    } __except (CheckHCDim (GetExceptionInformation (), 2))
    {
        // Restore original []CT
...
//  $GcdBA4F
//
//  GCD (Greatest Common Divisor) for BA4F
//***************************************************************************
...

{
    ...
    // Copy to temp vars
    arb4f_init_set (&aplLft, &aplBA4FLft);
    arb4f_init_set (&aplRht, &aplBA4FRht);
    ...
    __try
    {
        ...
        {
            // T = L
            arb4f_set (&aplTmp, &aplLft);

            // L = L | R
            aplMod = ModBA4F (aplLft, aplRht);
            arb4f_set (&aplLft, &aplMod);
            arb4f_clear (&aplMod);

            // R = T
            arb4f_set (&aplRht, &aplTmp);

            // Calculate the magnitude
            aplMagLft = MagBA4F (aplLft);
        ...
        // Make the left & right units
        MakeUnit (ARRAY_BA4F, uLft, &atLft);
        MakeUnit (ARRAY_BA4F, uRht, &atRht);

        // Multiply on the right by the righthand unit
        aplMulRht = MulBA4F_RE (aplRht, atRht.aplBA4F);

        // Multiply on the left by the lefthand unit
        aplMulLft = MulBA4F_RE (atLft.aplBA4F, aplMulRht);

        // Save in the result
        arb4f_set (&aplRht, &aplMulLft);

        // We no longer need these resources
        arb4f_clear (&atRht.aplBA4F);
        arb4f_clear (&atLft.aplBA4F);
        arb4f_clear (&aplMulLft    );
        arb4f_clear (&aplMulRht    );
        arb4f_clear (&aplTmp       );
////////arb4f_clear (&aplRht       );       // Returned as result
        arb4f_clear (&aplLft       );
    } __except (CheckHCDim (GetExceptionInformation (), 4))
    {
        // Restore original []CT
...
//  $PrimFnDydDownCaretBA1FisBA1FvBA1F
//
//  Primitive scalar function dyadic DownCaret:  BA1F {is} fn BA1F fn BA1F
//***************************************************************************

void PrimFnDydDownCaretBA1FisBA1FvBA1F
...

{
    ...
    else
    {
        ...
        if (arb_inf_p (&lpatLft->aplArb)
         || arb_inf_p (&lpatRht->aplArb))
            RaiseException (EXCEPTION_DOMAIN_ERROR, 0, 0, NULL);

        lpMemRes[uRes] = GcdBA1F (lpatLft->aplArb, lpatRht->aplArb);
    } // End IF/ELSE
} // End PrimFnDydDownCaretBA1FisBA1FvBA1F
...
//  $PrimFnDydDownCaretBA2FisBA2FvBA2F
//
//  Primitive scalar function dyadic DownCaret:  BA2F {is} BA2F fn BA2F
//***************************************************************************

void PrimFnDydDownCaretBA2FisBA2FvBA2F
...
{
    lpMemRes[uRes] = GcdBA2F (lpatLft->aplBA2F, lpatRht->aplBA2F);
} // End PrimFnDydDownCaretBA2FisBA2FvBA2F
...
//  $PrimFnDydDownCaretBA4FisBA4FvBA4F
//
//  Primitive scalar function dyadic DownCaret:  BA4F {is} BA4F fn BA4F
//***************************************************************************

void PrimFnDydDownCaretBA4FisBA4FvBA4F
...
{
    lpMemRes[uRes] = GcdBA4F (lpatLft->aplBA4F, lpatRht->aplBA4F);
} // End PrimFnDydDownCaretBA4FisBA4FvBA4F
