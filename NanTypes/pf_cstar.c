...
PRIMSPEC PrimSpecCircleStar =
{
    ...
    &PrimFnMonCircleStarBA1FisBA1F,
    &PrimFnMonCircleStarBA2FisBA2F,
    &PrimFnMonCircleStarBA4FisBA4F,
    &PrimFnMonCircleStarBA8FisBA8F,
    &PrimFnMonCircleStarBA1FisBA1F,
    NULL,   // &PrimFnMonCircleStarARBisBA2F, -- Can't happen w/CircleStar
    NULL,   // &PrimFnMonCircleStarARBisBA4F, -- Can't happen w/CircleStar
    NULL,   // &PrimFnMonCircleStarARBisBA8F, -- Can't happen w/CircleStar
    ...
    NULL,   // &PrimFnDydCircleStarBisBA1FvBA1F, -- Can't happen w/CircleStar
    NULL,   // &PrimFnDydCircleStarBisBA2FvBA2F, -- Can't happen w/CircleStar
    NULL,   // &PrimFnDydCircleStarBisBA4FvBA4F, -- Can't happen w/CircleStar
    NULL,   // &PrimFnDydCircleStarBisBA8FvBA8F, -- Can't happen w/CircleStar
    ...
    &PrimFnDydCircleStarBA1FisBA1FvBA1F,
    &PrimFnDydCircleStarBA2FisBA2FvBA2F,
    &PrimFnDydCircleStarBA4FisBA4FvBA4F,
    &PrimFnDydCircleStarBA8FisBA8FvBA8F,
...
//  $PrimFnMonCircleStarBA1FisBA1F
//
//  Primitive scalar function monadic CircleStar:  BA1F {is} fn BA1F
//
//  See "http://www.jsoftware.com/jwiki/Essays/Extended%20Precision%20Functions"
//***************************************************************************

void PrimFnMonCircleStarBA1FisBA1F
...

{
    ...
    // Check for Complex result
    if (arb_sign (&lpatRht->aplArb) < 0)  // Not SIGN_APLARB as that'll catch -0 whose log is indeterminate
    ...
    // Check for special cases:  {log} -_
    if (IsArbNegInfinity (&lpatRht->aplArb))
        RaiseException (EXCEPTION_DOMAIN_ERROR, 0, 0, NULL);
    else
    {
#if OWN_EXPLOG
...
////////log2x = (int) (floor (0.5 + (log (mpfr_get_d (&lpatRht->aplVfp)) / log (2.0))));
        ...
        if (mpfr_cmp (&lpMemRes[uRes], &mpfSqrt2) > 0)
        {
            mpfr_div (&mpfTmp, &lpMemRes[uRes], &mpfSqrt2, prec);
            mpfr_floor (&mpfTmp, &mpfTmp);
            mpz_set_fr (&mpzTmp, &mpfTmp);
            log2x = (int) mpz_sizeinbase (&mpzTmp, 2);
            mpfr_div_2exp (&lpMemRes[uRes], &lpMemRes[uRes], log2x, prec);
            log2xSign =  1;
        } else
        if (mpfr_cmp (&mpfInvSqrt2, &lpMemRes[uRes]) > 0)
        {
            mpfr_div (&mpfTmp, &mpfInvSqrt2, &lpMemRes[uRes], prec);
            mpfr_floor (&mpfTmp, &mpfTmp);
            mpz_set_fr (&mpzTmp, &mpfTmp);
            log2x = (int) mpz_sizeinbase (&mpzTmp, 2);
            mpfr_mul_2exp (&lpMemRes[uRes], &lpMemRes[uRes], log2x, prec);
            log2xSign = -1;
        } else
            log2x = log2xSign = 0;
        ...
        while (mpfr_cmp (&mpfInvSqrt2, &lpMemRes[uRes]) > 0)
        {
            mpfr_mul_ui (&lpMemRes[uRes], &lpMemRes[uRes], 2, prec);
            log2x++;
        } // End WHILE
        ...
        if (log2xSign < 0)
            mpfr_neg (&mpfTmp, &mpfTmp);
        mpfr_add    (&lpMemRes[uRes], &lpMemRes[uRes], &mpfTmp, prec);
...
//  $PrimFnMonCircleStarBA2FisBA2F
//
//  Primitive scalar function monadic CircleStar:  BA2F {is} fn BA2F
//
//  See "http://www.jsoftware.com/jwiki/Essays/Extended%20Precision%20Functions"
//***************************************************************************

void PrimFnMonCircleStarBA2FisBA2F
...
{
    // Call subfunction
    lpMemRes[uRes] = LogBAxF_RE (lpatRht->aplBA8F, 2).partsLo.partsLo;
} // End PrimFnMonCircleStarBA2FisBA2F
...
//  $PrimFnMonCircleStarBA4FisBA4F
//
//  Primitive scalar function monadic CircleStar:  BA4F {is} fn BA4F
//
//  See "http://www.jsoftware.com/jwiki/Essays/Extended%20Precision%20Functions"
//***************************************************************************

void PrimFnMonCircleStarBA4FisBA4F
...
{
    // Call subfunction
    lpMemRes[uRes] = LogBAxF_RE (lpatRht->aplBA8F, 4).partsLo;
} // End PrimFnMonCircleStarBA4FisBA4F
...
//  $PrimFnMonCircleStarBA8FisBA8F
//
//  Primitive scalar function monadic CircleStar:  BA8F {is} fn BA8F
//
//  See "http://www.jsoftware.com/jwiki/Essays/Extended%20Precision%20Functions"
//***************************************************************************

void PrimFnMonCircleStarBA8FisBA8F
...
{
    // Call subfunction
    lpMemRes[uRes] = LogBAxF_RE (lpatRht->aplBA8F, 8);
} // End PrimFnMonCircleStarBA8FisBA8F
...

{
    ...
    */
    ...
    // Calculate the magnitude of the imaginary parts
    arb_sqrt    (&aplIMag, &aplIMag, prec);                           // g
    ...
    if (!IsArb0 (&aplIMag))
    {
        APLARB aplMul = {0};            // Multiplier:  arctan2 (...) / ...

        // Initialize to 0/0
        Myarb_init (&aplMul);

        // Calculate arctan2 (aplIMag, aplRht.parts[0])
        arb_atan2 (&aplMul, &aplIMag, &aplRht.parts[0], prec);

        // Divide by the magnitude of the imaginary parts
        // Calculate arctan2 (aplIMag, aplRht.parts[0]) / aplIMag
        arb_div   (&aplMul, &aplMul, &aplIMag, prec);                 // v

        // Loop through the imaginary parts
        for (i = 1; i < iHCDimRes; i++)
        // Split cases based upon whether or not the Real and/or Imaginary parts are infinite
        switch (2 * arb_inf_p (&aplRht.parts[0]) + 1 * arb_inf_p (&aplRht.parts[i]))
        {
            case 2 * 0 + 1 * 0:         // Finite, finite
                // Multiply each of the imaginary parts by the arctan2
            ...
            case 2 * 0 + 1 * 1:         // Finite, infinite
                // Set the imaginary part to 켗i/2
                arb_init_set (&aplRes.parts[i], &lpMemPTD->arbBA8F_Pi.parts[0]);
                arb_div_ui   (&aplRes.parts[i], &aplRes.parts[i], 2, prec);

                if (arb_sign (&aplRht.parts[i]) < 0)
                    arb_neg (&aplRes.parts[i], &aplRes.parts[i]);

                break;

            case 2 * 1 + 1 * 0:         // Infinite, finite
                // Set the imaginary part to 0 or Pi depending upon the sign of the real part
                arb_init_set (&aplRes.parts[i], (arb_sign (&aplRht.parts[0]) < 0) ? &lpMemPTD->arbBA8F_Pi.parts[0] : &arbZero);

                break;

            case 2 * 1 + 1 * 1:         // Infinite, infinite
                // Copy Pi/4
                arb_set    (&aplMul, &lpMemPTD->arbBA8F_Pi.parts[0]);
                arb_div_ui (&aplMul, &aplMul, 4, prec);

                // Split cases based upon the signs of the real and imaginary parts
                switch ((2 * (arb_sign (&aplRht.parts[0]) < 0) + 1 * (arb_sign (&aplRht.parts[i]) < 0)))
                {
                    // Set the imaginary part to Pi/4 � N * Pi/2 depending upon the signs of the real and imaginary parts
                    case 2 * 0 + 1 * 0:         // Pos, Pos
////////////////////////aplRes.parts[i] =  1 * FloatPi / 4;
////////////////////////arb_mul_si (&aplMul, &aplMul,  1, prec);
                        arb_init_set (&aplRes.parts[i], &aplMul);

                        break;

                    case 2 * 1 + 1 * 0:         // Neg, Pos
////////////////////////aplRes.parts[i] =  3 * FloatPi / 4;
                        arb_mul_si (&aplMul, &aplMul,  3, prec);
                        arb_init_set (&aplRes.parts[i], &aplMul);
...
////////////////////////aplRes.parts[i] = -1 * FloatPi / 4;
    ...
    {
        // If the real part is < 0, ...
        if (arb_cmp_ui (&aplRht.parts[0], 0) < 0)
            // Point the first imaginary part to +180 degrees
            arb_init_set (&aplRes.parts[1], &lpMemPTD->arbBA8F_Pi.parts[0]);
        else
            // Point the first imaginary part to 0 degrees
...
////aplRMag = MulBA1F_RE (aplRht.parts[0], aplRht.parts[0]);
    ...
    // Calculate the magnitude of the entire arg and
    //   the magnitude of just the imaginary parts
    arb_sqrt    (&aplRMag, &aplRMag, prec);                           // m
    arb_sqrt    (&aplIMag, &aplIMag, prec);                           // g
    ...
    {
        // Calculate arctan2 (aplIMag, aplRht.parts[0]) / aplIMag
        arb_atan2 (&aplMul, &aplIMag, &aplRht.parts[0], prec);        // v

        // Divide by the magnitude of the imaginary parts
        arb_div   (&aplMul, &aplMul, &aplIMag, prec);
        ...
        if (IsArb0 (&aplMul))
        {
            ...
            {
                // If the right arg item is {Inf}, ...
                if (IsArbInfinity (&aplRht.parts[i]))
                {
                    // Set the corresponding item in the result to 켗i/2
                    arb_init_set (&aplRes.parts[i], &lpMemPTD->arbBA8F_Pi.parts[0]);
                    arb_div_ui   (&aplRes.parts[i], &aplRes.parts[i], 2, prec);
                    ...
                    // If the signs differ, ...
                    if (SIGN_APLARB_RAW (&aplRht.parts[i]) NE SIGN_APLARB_RAW (&aplMul))
                        // Negate the value
                        arb_neg (&aplRes.parts[i], &aplRes.parts[i]);
                } else
                {
                    arb_init_set_d (&aplRes.parts[i],
                                     bAllowNeg0 ? ((SIGN_APLARB_RAW (&aplRht.parts[i])
                                                 EQ SIGN_APLARB_RAW (&aplMul)) ? 0.0
                                                                               : -0.0)
                                                : 0.0);
                } // End IF/ELSE
            } // End FOR
...
//  $PrimFnDydCircleStarBA1FisBA1FvBA1F
//
//  Primitive scalar function dyadic CircleStar:  BA1F {is} BA1F fn BA1F
//***************************************************************************

void PrimFnDydCircleStarBA1FisBA1FvBA1F
    (LPAPLARB   lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...

{
    ...
    // Check for special cases:  PoM_ {log} 0
    // Check for special cases:  PoM_ {log} PoM_
    if ((IsArb0 (&lpatLft->aplArb) && arb_inf_p (&lpatRht->aplArb))
     || (arb_inf_p (&lpatLft->aplArb) && IsArb0 (&lpatRht->aplArb))
     || (arb_inf_p (&lpatLft->aplArb) && arb_inf_p (&lpatRht->aplArb)))
        RaiseException (EXCEPTION_DOMAIN_ERROR, 0, 0, NULL);
    else
    // Check for indeterminates:  B {log} B
...
} // End PrimFnDydCircleStarBA1FisBA1FvBA1F
...
APLBA2F BaseLogBA2F_RE
    (APLBA2F aplLft,                // Left arg
     APLBA2F aplRht)                // Right ...

{
    APLBA2F aplRes,                 // The result
            aplLogLft,              // Log left
            aplLogRht;              // ... right

    // Calculate the respective logs
    aplLogLft = LogBAxF_RE (*(LPAPLBA8F) &aplLft, 2).partsLo.partsLo;
    aplLogRht = LogBAxF_RE (*(LPAPLBA8F) &aplRht, 2).partsLo.partsLo;

    // log R to the base L is (ln R) / ln L
    aplRes = DivBA2F_RE (aplLogRht, aplLogLft);

    // We no longer need this storage
    arb2f_clear (&aplLogRht);
    arb2f_clear (&aplLogLft);
...
//  $PrimFnDydCircleStarBA2FisBA2FvBA2F
//
//  Primitive scalar function dyadic CircleStar:  BA2F {is} BA2F fn BA2F
//***************************************************************************

void PrimFnDydCircleStarBA2FisBA2FvBA2F
    (LPAPLBA2F  lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...
{
    // Call subfunction
    lpMemRes[uRes] = BaseLogBA2F_RE (lpatLft->aplBA2F, lpatRht->aplBA2F);
} // End PrimFnDydCircleStarBA2FisBA2FvBA2F
...
APLBA4F BaseLogBA4F_RE
    (APLBA4F aplLft,                // Left arg
     APLBA4F aplRht)                // Right ...

{
    APLBA4F aplRes,                 // The result
            aplLogLft,              // Log left
            aplLogRht;              // ... right

    // Calculate the respective logs
    aplLogLft = LogBAxF_RE (*(LPAPLBA8F) &aplLft, 4).partsLo;
    aplLogRht = LogBAxF_RE (*(LPAPLBA8F) &aplRht, 4).partsLo;

    // log R to the base L is (ln R) / ln L
    aplRes = DivBA4F_RE (aplLogRht, aplLogLft);

    // We no longer need this storage
    arb4f_clear (&aplLogRht);
    arb4f_clear (&aplLogLft);
...
//  $PrimFnDydCircleStarBA4FisBA4FvBA4F
//
//  Primitive scalar function dyadic CircleStar:  BA4F {is} BA4F fn BA4F
//***************************************************************************

void PrimFnDydCircleStarBA4FisBA4FvBA4F
    (LPAPLBA4F  lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...
{
    // Call subfunction
    lpMemRes[uRes] = BaseLogBA4F_RE (lpatLft->aplBA4F, lpatRht->aplBA4F);
} // End PrimFnDydCircleStarBA4FisBA4FvBA4F
...
APLBA8F BaseLogBA8F_RE
    (APLBA8F aplLft,                // Left arg
     APLBA8F aplRht)                // Right ...

{
    APLBA8F aplRes,                 // The result
            aplLogLft,              // Log left
            aplLogRht;              // ... right

    // Calculate the respective logs
    aplLogLft = LogBAxF_RE (*(LPAPLBA8F) &aplLft, 8);
    aplLogRht = LogBAxF_RE (*(LPAPLBA8F) &aplRht, 8);

    // log R to the base L is (ln R) / ln L
    aplRes = DivBA8F_RE (aplLogRht, aplLogLft);

    // We no longer need this storage
    arb8f_clear (&aplLogRht);
    arb8f_clear (&aplLogLft);
...
//  $PrimFnDydCircleStarBA8FisBA8FvBA8F
//
//  Primitive scalar function dyadic CircleStar:  BA8F {is} BA8F fn BA8F
//***************************************************************************

void PrimFnDydCircleStarBA8FisBA8FvBA8F
    (LPAPLBA8F  lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...
{
    // Call subfunction
    lpMemRes[uRes] = BaseLogBA8F_RE (lpatLft->aplBA8F, lpatRht->aplBA8F);
} // End PrimFnDydCircleStarBA8FisBA8FvBA8F
