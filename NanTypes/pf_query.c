...
PRIMSPEC PrimSpecQuery =
{
    ...
    &PrimFnMonQueryBA1FisBA1F,
    NULL,   // &PrimFnMonQueryBA2FisBA2F,
    NULL,   // &PrimFnMonQueryBA4FisBA4F,
    NULL,   // &PrimFnMonQueryBA8FisBA8F,
    &PrimFnMonQueryBA1FisBA1F,
    NULL,   // &PrimFnMonQueryARBisBA2F,
    NULL,   // &PrimFnMonQueryARBisBA4F,
    NULL,   // &PrimFnMonQueryARBisBA8F,
    ...
    NULL,   // &PrimFnDydQueryBisBA1FvBA1F, -- Can't happen w/Query
    NULL,   // &PrimFnDydQueryBisBA2FvBA2F, -- Can't happen w/Query
    NULL,   // &PrimFnDydQueryBisBA4FvBA4F, -- Can't happen w/Query
    NULL,   // &PrimFnDydQueryBisBA8FvBA8F, -- Can't happen w/Query
    ...
    NULL,   // &PrimFnDydQueryBA1FisBA1FvBA1F, -- Dyadic Query not scalar
    NULL,   // &PrimFnDydQueryBA2FisBA2FvBA2F, -- Dyadic Query not scalar
    NULL,   // &PrimFnDydQueryBA4FisBA4FvBA4F, -- Dyadic Query not scalar
    NULL,   // &PrimFnDydQueryBA8FisBA8FvBA8F, -- Dyadic Query not scalar
...

{
    ...
    if (mpfr_cmp_ui (&lpatRht->aplVfp, 0) EQ 0)
    {
        ...
        // Generate the random number
        mpfr_urandomb (&lpMemRes[uRes], GetMemPTD ()->randStateGMP);
    } else
    // Check for DOMAIN ERROR
...
//  $PrimFnMonQueryBA1FisBA1F
//
//  Primitive scalar function monadic Query:  BA1F {is} fn BA1F
//***************************************************************************

void PrimFnMonQueryBA1FisBA1F
...

{
  ...
  #define lpMemPTD  GetMemPTD ()
    ...
    // Check for special value
    if (arb_cmp_ui (&lpatRht->aplArb, 0) EQ 0)
    {
        // Initialize the result to 0/0
        Myarb_init (&lpMemRes[uRes]);

        // Generate the random number in [0, 1)
        arb_urandomb (&lpMemRes[uRes], lpMemPTD->randStateFLINT);
    } else
    // Check for DOMAIN ERROR
    if (arb_cmp_ui (&lpatRht->aplArb, bQuadIO) < 0)
        RaiseException (EXCEPTION_DOMAIN_ERROR, 0, 0, NULL);
    else
    // If the ARB is an integer,  ...
    if (arb_integer_p (&lpatRht->aplArb))
    {
        ...
        switch (lpMemPTD->lphtsPTD->lpSymQuad[SYSVAR_DT]->stData.stChar)
        {
            case L'r':
                // Generate a uniformly-distributed random number in [0, Rht)
                arb_urandomm (&lpMemRes[uRes],
                              &lpatRht->aplArb,
                               lpMemPTD->randStateFLINT);
                // Add in []IO
                arb_add_ui (&lpMemRes[uRes],
                            &lpMemRes[uRes],
                             bQuadIO,
                             prec);
                break;
...

{
    ...
    if (bRet && IsRealArb (aplTypeLft))
        // Attempt to convert the ARB to an integer using System []CT
        aplIntegerLft = arb_get_sctsx  ((LPAPLARB) lpSymGlbLft, &bRet);
    if (bRet && IsRealArb (aplTypeRht))
        // Attempt to convert the ARB to an integer using System []CT
        aplIntegerRht = arb_get_sctsx  ((LPAPLARB) lpSymGlbRht, &bRet);
