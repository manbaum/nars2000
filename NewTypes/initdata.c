...

{
    ...
    hGlbRes = GlobalAlloc (GHND, (APLU3264) ByteRes);
...
////    goto WSFULL_EXIT;
    ...
    // Lock the memory to get a ptr to it
    lpaplInt = GlobalLock (lpMemComb[0]);

    // Save the value
    lpaplInt[0]  = 1;

    // We no longer need this ptr
    GlobalUnlock (lpMemComb[0]); lpaplInt = NULL;

    // We no longer need this ptr
    GlobalUnlock (hGlbRes); lpMemComb = NULL;
...
////    goto WSFULL_EXIT;
    ...
    // Lock the memory to get a ptr to it
    lpaplRat = GlobalLock (lpMemComb[0]);

    // Save the value
    mpq_init_set (&lpaplRat[0], &mpqOne);

    // We no longer need this ptr
    GlobalUnlock (lpMemComb[0]); lpaplRat = NULL;
...
////EnterCriticalSection (&CSOCombPNI);
    ...
    GlobalFree (ghGlbPNI); ghGlbPNI = NULL;
...
////EnterCriticalSection (&CSOCombPNR);
    ...
    for (i = 0; i < gCurLenPNR; i++)
    {
        ...
        lpMemPart = GlobalLock (lpMemComb[i]);
        ...
        // We no longer need this ptr
        GlobalUnlock (lpMemComb[i]); lpMemPart = NULL;
    ...
    GlobalFree (ghGlbPNR); ghGlbPNR = NULL;
...
////EnterCriticalSection (&CSOCombSN2I);
    ...
    GlobalFree (ghGlbSN2I); ghGlbSN2I = NULL;
...
////EnterCriticalSection (&CSOCombSN2R);
    ...
    GlobalFree (ghGlbSN2R); ghGlbSN2R = NULL;
...
////                                                                                        // Alt-'h' - delta
    ...
    InitPrimProtoOp (INDEX_OPDOUBLESHRIEK      , &PrimProtoOpCombinatorial_EM_YY     );     // Alt-'k' - combinatorial
////                                                                                        // Alt-'l' - quad
