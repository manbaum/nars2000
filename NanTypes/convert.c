...
//  $ConvertArbToInt
//
// Attempt to convert the ARB to an integer using []CT as called for by lpAllTypes->enumCT
//***************************************************************************

APLINT ConvertArbToInt
    (LPAPLBA1F  lpaplArb,
     APLINT     uInt,
     LPALLTYPES lpAllTypes,
     LPUBOOL    lpbRet)

{
    ...
    if (mag_equal (arb_radref (&lpaplArb[uInt]), arb_radref (&arbZero)) EQ 0)
    {
        if (lpbRet NE NULL)
            *lpbRet = FALSE;
        return 0;
    ...
    {
        case ENUMCT_NONE:
            // Attempt to convert the ARB to an integer using NO CT
            return _arb_get_ctsx (&lpaplArb[uInt], 0.0                  , lpbRet, FALSE);

        case ENUMCT_USER:
            // Attempt to convert the ARB to an integer using User CT
            return _arb_get_ctsx (&lpaplArb[uInt], lpAllTypes->fQuadCT  , lpbRet, FALSE);

        case ENUMCT_SYS:
            // Attempt to convert the ARB to an integer using System CT
            return _arb_get_ctsx (&lpaplArb[uInt], SYS_CT               , lpbRet, TRUE);
...
//  $ConvertRatToArb
//
// Attempt to convert the RAT to an ARB
//***************************************************************************

APLARB ConvertRatToArb
    (LPAPLRAT   lpaplRat,
     APLINT     uInt)

{
    ...
    // Convert the RAT to FMPQ
    fmpq_set_mpq (fmpq, &lpaplRat[uInt]);

    // Convert the FMPQ to ARB
    arb_set_fmpq (&aplRes, fmpq, prec);
...
//  $ARBToAplint
//
//  Attempt to convert a ARB number to an APLINT
//    using System Comparison Tolerance
//***************************************************************************

APLINT ARBToAplint
    (LPAPLARB   lpDataLft,      // Ptr to left arg data
     APLUINT    u,              // Index into left arg
     LPUBOOL    lpbRet)         // Ptr to TRUE iff it succeeded

{
    // Attempt to convert the ARB to an integer using System []CT
    return arb_get_sctsx (&lpDataLft[u], lpbRet);
} // ARBToAplint
...
//  $BA2FToAplint_SCT
//
//  Attempt to convert a BA2F number to an APLINT
//    using System Comparison Tolerance
//***************************************************************************

APLINT BA2FToAplint_SCT
    (LPAPLBA2F  lpDataLft,      // Ptr to left arg data
     APLUINT    u,              // Index into left arg
     LPUBOOL    lpbRet)         // Ptr to TRUE iff it succeeded

{
    ...
    *lpbRet = TRUE;
    ...
    // Loop through the imaginary parts
    for (i = 1; i < 2; i++)
        // Determine if the imaginary parts of BA2F are all near zero
        *lpbRet &= arb_get_sctsx (&lpDataLft[u].parts[1], lpbRet) < SYS_CT;

    // Get the next dimension
    return arb_get_sctsx (&lpDataLft[u].parts[0], lpbRet);
} // BA2FToAplint_SCT
...
//  $BA4FToAplint_SCT
//
//  Attempt to convert a BA4F number to an APLINT
//    using System Comparison Tolerance
//***************************************************************************

APLINT BA4FToAplint_SCT
    (LPAPLBA4F  lpDataLft,      // Ptr to left arg data
     APLUINT    u,              // Index into left arg
     LPUBOOL    lpbRet)         // Ptr to TRUE iff it succeeded

{
    ...
    *lpbRet = TRUE;
    ...
    // Loop through the imaginary parts
    for (i = 1; i < 4; i++)
        // Determine if the imaginary parts of BA4F are all near zero
        *lpbRet &= arb_get_sctsx (&lpDataLft[u].parts[1], lpbRet) < SYS_CT;
...
//  $BA8FToAplint_SCT
//
//  Attempt to convert a BA8F number to an APLINT
//    using System Comparison Tolerance
//***************************************************************************

APLINT BA8FToAplint_SCT
    (LPAPLBA8F  lpDataLft,      // Ptr to left arg data
     APLUINT    u,              // Index into left arg
     LPUBOOL    lpbRet)         // Ptr to TRUE iff it succeeded

{
    ...
    *lpbRet = TRUE;
    ...
    // Loop through the imaginary parts
    for (i = 1; i < 8; i++)
        // Determine if the imaginary parts of BA8F are all near zero
        *lpbRet &= arb_get_sctsx (&lpDataLft[u].parts[1], lpbRet) < SYS_CT;
...

{
    ...
    switch (aplTypeCom)
    {
        ...
        case ARRAY_ARB:
            if (fQuadCT EQ 0.0)
                return signumint (arb_cmp (&lpatLft->aplArb, &lpatRht->aplArb));
            else
                return signumint (CmpCT_B ( lpatLft->aplArb,  lpatRht->aplArb, fQuadCT, -));
        ...
        case ARRAY_BA2F:
            return ba2f_cmp  (lpatLft->aplBA2F    , lpatRht->aplBA2F   , fQuadCT);

        case ARRAY_BA4F:
            return ba4f_cmp  (lpatLft->aplBA4F    , lpatRht->aplBA4F   , fQuadCT);

        case ARRAY_BA8F:
            return ba8f_cmp  (lpatLft->aplBA8F    , lpatRht->aplBA8F   , fQuadCT);
...
int ba4f_cmp
    (APLBA4F  aplBA4FLft,           // Left arg float
     APLBA4F  aplBA4FRht,           // Right ...
     APLFLOAT fQuadCT)              // []CT (0 = exact comparison)
