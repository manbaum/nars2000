...
PRIMSPEC PrimSpecEqual = {
    ...
    NULL,   // &PrimFnMonEqualBA1FisBA1F,
    NULL,   // &PrimFnMonEqualBA2FisBA2F,
    NULL,   // &PrimFnMonEqualBA4FisBA4F,
    NULL,   // &PrimFnMonEqualBA8FisBA8F,
    NULL,   // &PrimFnMonEqualARBisBA1F,
    NULL,   // &PrimFnMonEqualARBisBA2F,
    NULL,   // &PrimFnMonEqualARBisBA4F,
    NULL,   // &PrimFnMonEqualARBisBA8F,
    ...
    &PrimFnDydEqualBisBA1FvBA1F,
    &PrimFnDydEqualBisBA2FvBA2F,
    &PrimFnDydEqualBisBA4FvBA4F,
    &PrimFnDydEqualBisBA8FvBA8F,
    ...
    NULL,   // &PrimFnDydEqualBA1FisBA1FvBA1F,
    NULL,   // &PrimFnDydEqualBA2FisBA2FvBA2F,
    NULL,   // &PrimFnDydEqualBA4FisBA4FvBA4F,
    NULL,   // &PrimFnDydEqualBA8FisBA8FvBA8F,
...
//  $PrimFnDydEqualBisBA1FvBA1F
//
//  Primitive scalar function dyadic Equal:  B {is} BA1F fn BA1F
//***************************************************************************

void PrimFnDydEqualBisBA1FvBA1F
    (LPAPLBOOL  lpMemRes,               // Ptr to the result
     APLUINT    uRes,                   // Index into the result
     LPALLTYPES lpatLft,                // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,                // ...    right ...
...

{
    // Compare the two VFPs relative to []CT
    lpMemRes[uRes >> LOG2NBIB] |=
      CmpCT_B (lpatLft->aplArb, lpatRht->aplArb, GetQuadCT (), EQ)
      << (MASKLOG2NBIB & (UINT) uRes);
} // End PrimFnDydEqualBisBA1FvBA1F
...
UBOOL EqualBAxFvBAxF
    (LPALLTYPES lpatLft,                // Left arg as LPALLTYPES
     ...
     LPWCHAR    lpwszMsg)               // Ptr to GetExceptionInformation msg
{
    APLARB aplMagLft,                   // Magnitude of left arg
           aplMagRht,                   // ...          right ...
           aplMagMax = {0},             // Magnitude of larger
           aplDist;                     // Distance between left and right args
    UBOOL  bRet;                        // The result
    ...
    __try
    {
        ...
        {
            case 1:
                return CmpCT_B (lpatLft->aplArb, lpatRht->aplArb, fQuadCT, EQ);

            case 2:
                // Calculate the magnitudes
                aplMagLft = MagBA2F (lpatLft->aplBA2F);
                aplMagRht = MagBA2F (lpatLft->aplBA2F);

                // If neither of the magnitudes are infinite, ...
                if (!IsArbInfinity (&aplMagLft) && !IsArbInfinity (&aplMagRht))
                    // Get the distance between the args
                    aplDist   = DistBA2F (lpatLft->aplBA2F, lpatRht->aplBA2F);

                break;

            case 4:
                // Calculate the magnitudes
                aplMagLft = MagBA4F (lpatLft->aplBA4F);
                aplMagRht = MagBA4F (lpatLft->aplBA4F);

                // If neither of the magnitudes are infinite, ...
                if (!IsArbInfinity (&aplMagLft) && !IsArbInfinity (&aplMagRht))
                    // Get the distance between the args
                    aplDist   = DistBA4F (lpatLft->aplBA4F, lpatRht->aplBA4F);

                break;

            case 8:
                // Calculate the magnitudes
                aplMagLft = MagBA8F (lpatLft->aplBA8F);
                aplMagRht = MagBA8F (lpatLft->aplBA8F);

                // If neither of the magnitudes are infinite, ...
                if (!IsArbInfinity (&aplMagLft) && !IsArbInfinity (&aplMagRht))
                    // Get the distance between the args
                    aplDist   = DistBA8F (lpatLft->aplBA8F, lpatRht->aplBA8F);
        ...
        // Calculate the larger magnitude
        if (arb_cmp (&aplMagLft, &aplMagRht) < 0)
            arb_init_set (&aplMagMax, &aplMagRht);
        else
            arb_init_set (&aplMagMax, &aplMagLft);

        // Multiply it by []CT
        arb_mul_d (&aplMagMax, &aplMagMax, fQuadCT);
    } __except (CheckException (GetExceptionInformation (), lpwszMsg))
    {
        EXCEPTION_CODES exCode = MyGetExceptionCode ();  // The exception code
    ...
    // If both of the magnitudes are infinite or NaNs, ...
    if ((IsArbInfinity (&aplMagLft) && IsArbInfinity (&aplMagRht))
     || (arf_is_nan    (arb_midref (&aplMagLft)) && arf_is_nan (arb_midref (&aplMagRht))))
    {
        // Compare the individual coordinates
        for (i = 0; i < iHCDim; i++)
        if (CmpCT_B (lpatLft->aplBA8F.parts[i],
                     lpatRht->aplBA8F.parts[i],
                     fQuadCT,
                     NE))
            bRet = FALSE;
    } else
    // If one of the magnitudes is infinite and the other isn't, ...
    if (IsArbInfinity (&aplMagLft) NE IsArbInfinity (&aplMagRht))
        bRet = FALSE;
    else
        // The two numbers are equal iff the Dist is <= []CT * max (aplMagLft, aplMagRht)
        bRet = arb_cmp (&aplDist, &aplMagMax) <= 0;

    // We no longer need this resource
    Myarb_clear (&aplMagMax);
...
{
    // The two numbers are equal iff the Dist is <= []CT * max (aplMagLft, aplMagRht)
    if (bIsEqual EQ EqualBAxFvBAxF (lpatLft, lpatRht, iHCDim, GetQuadCT (), lpwszMsg))
        lpMemRes[uRes >> LOG2NBIB] |=
          BIT0
          << (MASKLOG2NBIB & (UINT) uRes);
} // End EqualBisBAxFvBAxF


//***************************************************************************
//  $PrimFnDydEqualBisBA2FvBA2F
//
//  Primitive scalar function dyadic Equal:  B {is} BA2F fn BA2F
//***************************************************************************

void PrimFnDydEqualBisBA2FvBA2F
    (LPAPLBOOL  lpMemRes,               // Ptr to the result
     APLUINT    uRes,                   // Index into the result
     LPALLTYPES lpatLft,                // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,                // ...    right ...
...
{
    EqualBisBAxFvBAxF (lpMemRes,        // Ptr to the result
                       uRes,            // Index into the result
                       lpatLft,         // Left arg as LPALLTYPES
                       lpatRht,         // Right ...
                       2,               // Dimension (1, 2, 4, 8)
                       TRUE,            // TRUE iff the calling function is Equal (FALSE if NotEqual)
                       WFCN);   // Ptr to CheckException msg
} // End PrimFnDydEqualBisBA2FvBA2F


//***************************************************************************
//  $PrimFnDydEqualBisBA4FvBA4F
//
//  Primitive scalar function dyadic Equal:  B {is} BA4F fn BA4F
//***************************************************************************

void PrimFnDydEqualBisBA4FvBA4F
    (LPAPLBOOL  lpMemRes,               // Ptr to the result
     APLUINT    uRes,                   // Index into the result
     LPALLTYPES lpatLft,                // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,                // ...    right ...
...

{
    EqualBisBAxFvBAxF (lpMemRes,        // Ptr to the result
                       ...
                       TRUE,            // TRUE iff the calling function is Equal (FALSE if NotEqual)
                       WFCN);   // Ptr to CheckException msg
} // End PrimFnDydEqualBisBA4FvBA4F


//***************************************************************************
//  $PrimFnDydEqualBisBA8FvBA8F
//
//  Primitive scalar function dyadic Equal:  B {is} BA8F fn BA8F
//***************************************************************************

void PrimFnDydEqualBisBA8FvBA8F
    (LPAPLBOOL  lpMemRes,               // Ptr to the result
     APLUINT    uRes,                   // Index into the result
     LPALLTYPES lpatLft,                // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,                // ...    right ...
...

{
    EqualBisBAxFvBAxF (lpMemRes,        // Ptr to the result
                       ...
                       TRUE,            // TRUE iff the calling function is Equal (FALSE if NotEqual)
                       WFCN);   // Ptr to CheckException msg
} // End PrimFnDydEqualBisBA8FvBA8F
