...
////									 $$ = *lpYYStr;}
	...
	| ArrExpr ASSIGN	   QUAD 		{DbgMsgW2 (L"%%SimpExpr:  " WS_UCS2_QUAD WS_UCS2_LEFTARROW L"ArrExpr");
										 bRet = ArrayDisplay_EM (&$1.tkToken);
										 if (!bRet)
										 {
											 FreeResult (&$1.tkToken);
	...
	| ArrExpr ASSIGN	   QUOTEQUAD	{DbgMsgW2 (L"%%SimpExpr:  " WS_UCS2_QUOTEQUAD WS_UCS2_LEFTARROW L"ArrExpr");
...

{
	...
	switch (gplLocalVars.lpNext->tkFlags.TknType)
	{
		...
		case TKT_INPOUT:
			if (gplLocalVars.lpNext->tkData.tkChar EQ UCS2_QUAD)
				return QUAD;
			else
				return QUOTEQUAD;
