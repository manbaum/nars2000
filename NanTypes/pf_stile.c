...
PRIMSPEC PrimSpecStile = {
    ...
    &PrimFnMonStileBA1FisBA1F,
    &PrimFnMonStileBA2FisBA2F,
    &PrimFnMonStileBA4FisBA4F,
    &PrimFnMonStileBA8FisBA8F,
    &PrimFnMonStileBA1FisBA1F,
    &PrimFnMonStileARBisBA2F,
    &PrimFnMonStileARBisBA4F,
    &PrimFnMonStileARBisBA8F,
    ...
    NULL,   // &PrimFnDydStileBisBA1FvBA1F, -- Can't happen w/Stile
    NULL,   // &PrimFnDydStileBisBA2FvBA2F, -- Can't happen w/Stile
    NULL,   // &PrimFnDydStileBisBA4FvBA4F, -- Can't happen w/Stile
    NULL,   // &PrimFnDydStileBisBA8FvBA8F, -- Can't happen w/Stile
    ...
    &PrimFnDydStileBA1FisBA1FvBA1F,
    &PrimFnDydStileBA2FisBA2FvBA2F,
    &PrimFnDydStileBA4FisBA4FvBA4F,
    NULL,   // &PrimFnDydStileBA8FisBA8FvBA8F, -- Can't happen w/Stile
...

{
    ...
    // Calculate the absolute value of an ARB
    arb_abs2 (&aplRes, &aplRht);
...
//  $PrimFnMonStileBA1FisBA1F
//
//  Primitive scalar function monadic Stile:  BA1F {is} fn BA1F
//***************************************************************************

void PrimFnMonStileBA1FisBA1F
...
{
    // Calculate the absolute value of an ARB
    lpMemRes[uRes] = MagArb (lpatRht->aplArb);
} // End PrimFnMonStileBA1FisBA1F
...

{
    ...
    // Sum the squares
    aplRes = SqNrmBA2F (&aplRht);

    // Get the magnitude of the arg
    arb_sqrt    (&aplRes, &aplRes, prec);
...
//  $PrimFnMonStileBA2FisBA2F
//
//  Primitive scalar function monadic Stile:  BA2F {is} fn BA2F
//***************************************************************************

void PrimFnMonStileBA2FisBA2F
...

{
    ...
    // Calculate magnitude
    aplMag = MagBA2F (lpatRht->aplBA2F);

    // Call subfunction, setting real part only
    arb_set (&lpMemRes[uRes].parts[0], &aplMag);
} // End PrimFnMonStileBA2FisBA2F
...
//  $PrimFnMonStileBA4FisBA4F
//
//  Primitive scalar function monadic Stile:  BA4F {is} fn BA4F
//***************************************************************************

void PrimFnMonStileBA4FisBA4F
...

{
    ...
    // Calculate magnitude
    aplMag = MagBA4F (lpatRht->aplBA4F);

    // Call subfunction, setting real part only
    arb_set (&lpMemRes[uRes].parts[0], &aplMag);
} // End PrimFnMonStileBA4FisBA4F
...
//  $PrimFnMonStileBA8FisBA8F
//
//  Primitive scalar function monadic Stile:  BA8F {is} fn BA8F
//***************************************************************************

void PrimFnMonStileBA8FisBA8F
...

{
    ...
    // Calculate magnitude
    aplMag = MagBA8F (lpatRht->aplBA8F);

    // Call subfunction, setting real part only
    arb_set (&lpMemRes[uRes].parts[0], &aplMag);
} // End PrimFnMonStileBA8FisBA8F
...
{
    // Save in the result
    lpMemRes[uRes] = MagBA2F (lpatRht->aplBA2F);
} // End PrimFnMonStileARBisBA2F
...
{
    // Save in the result
    lpMemRes[uRes] = MagBA4F (lpatRht->aplBA4F);
} // End PrimFnMonStileARBisBA4F
...
{
    // Save in the result
    lpMemRes[uRes] = MagBA8F (lpatRht->aplBA8F);
} // End PrimFnMonStileARBisBA8F
...
APLBA1F ModBA1F
    (APLBA1F aplLft,                // Left arg (modulus)
...

{
    ...
    {
        if (bNaNLft)
            arb_set (&aplRes, &aplLft);
        else
            arb_set (&aplRes, &aplRht);
        // Skip the ending sign-setting code as
        //   there is only one NaN
        bNoSign = TRUE;
    } else
    ...
    } else
    {
        ...
        // Ensure both arguments are non-negative
        arb_abs2 (&aplFlr, &aplLft);
        arb_abs2 (&aplCel, &aplRht);

        // Calculate right divided-by left
        arb_div (&aplTmp, &aplCel, &aplFlr, prec);
...
//  $PrimFnDydStileBA1FisBA1FvBA1F
//
//  Primitive scalar function dyadic Stile:  BA1F {is} BA1F fn BA1F
//***************************************************************************

void PrimFnDydStileBA1FisBA1FvBA1F
    (LPAPLARB   lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...
{
    // Save in the result
    lpMemRes[uRes] = ModBA1F (lpatLft->aplBA1F, lpatRht->aplBA1F);
} // End PrimFnDydStileBA1FisBA1FvBA1F
...
APLBA2F ModBA2F
    (APLBA2F aplLft,                // Left arg (modulus)
     APLBA2F aplRht)                // Right arg (argument)

{
    ...
    // Is either arg a NaN?
    bNaNLft = IsArgNaN (ARRAY_BA2F, &aplLft, 0);
    bNaNRht = IsArgNaN (ARRAY_BA2F, &aplRht, 0);
    ...
    {
        if (bNaNLft)
            arb2f_set (&aplRes, &aplLft);
        else
            arb2f_set (&aplRes, &aplRht);
    } else
    if (IsOneBAxF  (&aplLft, 2))
        aplRes = SubBA2F_RE (aplRht, FloorBA2F (aplRht));
    else
    {
        APLBA2F aplTmp = {0},
                aplHCFlr = {0},
        ...
        // Calculate Rht / Lft
        aplTmp = DivBA2F_RE (aplRht, aplLft);

        // Calculate Floor (Rht / Lft)
        aplHCFlr = FloorBA2F (aplTmp);

        // Calculate aplTmp EQ Floor (aplTmp)
        if (EqualBAxFvBAxF ((LPALLTYPES) &aplTmp, (LPALLTYPES) &aplHCFlr, 2, GetQuadCT (), WFCN))
            // Set aplTmp to 0/0
            arb2f_init (&aplSub);
        else
            // Calculate 1 | Rht / Lft
            // a.k.a. (Rht / Lft) - floor (Rht / Lft)
            aplSub = SubBA2F_RE (aplTmp, aplHCFlr);

        // Calculate Lft * 1 | Rht / Lft
        // Note that because Complex numbers are commutative,
        //   we don't need to be sensitive to []LR in the
        //   following multiplication
        aplRes = MulBA2F_RE (aplLft, aplSub);

        // Free the temps
        arb2f_clear (&aplSub);
        arb2f_clear (&aplHCFlr);
        arb2f_clear (&aplTmp);
    } // End IF/ELSE/...
...
//  $PrimFnDydStileBA2FisBA2FvBA2F
//
//  Primitive scalar function dyadic Stile:  BA2F {is} BA2F fn BA2F
//***************************************************************************

void PrimFnDydStileBA2FisBA2FvBA2F
    (LPAPLBA2F  lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...
{
    // Save in the result
    lpMemRes[uRes] = ModBA2F (lpatLft->aplBA2F, lpatRht->aplBA2F);
} // End PrimFnDydStileBA2FisBA2FvBA2F
...
APLBA4F ModBA4F
    (APLBA4F aplLft,                // Left arg (modulus)
     APLBA4F aplRht)                // Right arg (argument)

{
    ...
    // Is either arg a NaN?
    bNaNLft = IsArgNaN (ARRAY_BA4F, &aplLft, 0);
    bNaNRht = IsArgNaN (ARRAY_BA4F, &aplRht, 0);
    ...
    {
        if (bNaNLft)
            arb4f_set (&aplRes, &aplLft);
        else
            arb4f_set (&aplRes, &aplRht);
    } else
    if (IsOneBAxF  (&aplLft, 4))
        aplRes = SubBA4F_RE (aplRht, FloorBA4F (aplRht));
    else
    {
        APLBA4F aplTmp = {0},
                aplHCFlr = {0},
        ...
        // Calculate Floor (Rht / Lft)
        aplHCFlr = FloorBA4F (aplTmp);

        // Calculate aplTmp EQ Floor (aplTmp)
        if (EqualBAxFvBAxF ((LPALLTYPES) &aplTmp, (LPALLTYPES) &aplHCFlr, 4, GetQuadCT (), WFCN))
            // Set aplTmp to 0/0
            arb4f_init (&aplSub);
        else
            // Calculate 1 | Rht / Lft
            // a.k.a. (Rht / Lft) - floor (Rht / Lft)
            aplSub = SubBA4F_RE (aplTmp, aplHCFlr);

        if (GetQuadLR () EQ 'l')
            // Calculate Lft * 1 | Rht / Lft
            aplRes = MulBA4F_RE (        aplLft, aplSub);
        else
            // Calculate      (1 | Rht / Lft) * Lft
            aplRes = MulBA4F_RE (aplSub, aplLft);
        ...
        {
            // Get the absolute value
            arb_abs2 (&aplAbs, &aplRes.parts[i]);

            // Get the floor and ceiling
            Myarb_floor (&aplFlr, &aplAbs);
            Myarb_ceil  (&aplCel, &aplAbs);

            // Add/subtract 0.5 to/from Floor/Ceiling
            arb_add (&aplFlr, &aplFlr, &arbHalf, prec);
            arb_sub (&aplCel, &aplCel, &arbHalf, prec);

            // If floor (aplTmp) is near a Hurwitz integer within SYS_CT, ...
            if (CmpSCT_B (aplAbs, aplFlr, SYS_CT, EQ))
                // Save the half integer
                arb_set (&aplRes2.parts[i], &aplFlr);
            else
            // If ceil  (aplTmp) is near a Hurwitz integer within SYS_CT, ...
            if (CmpSCT_B (aplAbs, aplCel, SYS_CT, EQ))
                // Save the half integer
                arb_set (&aplRes2.parts[i], &aplCel);
            else
                break;
        } // End IF
        ...
        if (i EQ 4)
            ...
            {
                // Save the sign of aplRes
                if (arb_sign (&aplRes.parts[i]) < 0)
                    // Negate it
                    arb_neg (&aplRes2.parts[i], &aplRes2.parts[i]);

                // Copy aplRes2 to aplRes with the sign of aplRes
                arb_set (&aplRes.parts[i], &aplRes2.parts[i]);
            } // End FOR
        else
        // We did terminate early, so it's not a Hurwitz Integer
            // Loop through all of the parts
            for (i = 0; i < 4; i++)
            // If it's close to 0
            if (CmpSCT_B (aplRes.parts[i], arbZero, SYS_CT, EQ))
                // Call it 0/1
                arb_zero (&aplRes.parts[i]);
        // Calculate the magnitudes
        aplMagL = MagBA4F (aplLft);
        aplMagZ = MagBA4F (aplRes);

        // If the magnitudes are equal, ...
        if (CmpCT_B (aplMagL, aplMagZ, SYS_CT, EQ))
            // Return 0/1
            arb4f_set (&aplRes, &arb4f_0);
        else
            Assert (CmpCT_B (aplMagL, aplMagZ, SYS_CT, >));
...
//  $PrimFnDydStileBA4FisBA4FvBA4F
//
//  Primitive scalar function dyadic Stile:  BA4F {is} BA4F fn BA4F
//***************************************************************************

void PrimFnDydStileBA4FisBA4FvBA4F
    (LPAPLBA4F  lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...
{
    // Save in the result
    lpMemRes[uRes] = ModBA4F (lpatLft->aplBA4F, lpatRht->aplBA4F);
} // End PrimFnDydStileBA4FisBA4FvBA4F
