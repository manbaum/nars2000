...
////////lpYYRes->tkToken.tkFlags.NoDisplay = FALSE;         // Already zero from YYAlloc
    ...
    } else
    {
        ...
        // APLHETERO/APLNESTED/APLRAT/APLVFP/APLARB vs. anything
        // anything            vs. APLHETERO/APLNESTED/APLRAT/APLVFP/APLARB
...

{
    ...
    switch (aplTypeCom)
    {
        ...
        case ARRAY_BA4F:
        case ARRAY_BA8F:
            return EqualBAxFvBAxF (lpatLft, lpatRht, iHCDimX, fQuadCT, lpwszMsg);
...
#define GetNextARB(lpMem,typeMem,uRes)      ((LPAPLARB) lpMem)[uRes]
...
#define CompareARB(Lft,typeLft,Rht,typeRht) (CmpCT_B (&Lft, &Rht, fQuadCT, EQ))
     ...
     APLINT      kmpNext[])
{
    ...
    while (i < m)
    {
        ...
        while (j > -1 && !EqualBAxFvBAxF (&atLft, &atRht, iHCDimX, fQuadCT, WFCN))
        {
            j = kmpNext[j];
        ...
        if ((i < m) && EqualBAxFvBAxF (&atLft, &atRht, iHCDimX, fQuadCT, WFCN))
            kmpNext[i] = kmpNext[j];
        else
            kmpNext[i] = j;
    } // End WHILE
     ...
     APLINT      kmpNext[])
{
    ...
    switch (aplTypeLft)
    {
        ...
        case ARRAY_BA4F:
        case ARRAY_BA8F:
            preKmpBA (aplTypeCom, x, aplTypeLft, m, iHCDimX, fQuadCT, kmpNext);
...

{
    ...
    // Is the left arg RAT, VFP, ARB, or Other?
    uLft = (IsRealRat (aplTypeLft) ? 0
                               : (IsRealVfp (aplTypeLft) ? 1
                                                         : IsRealArb (aplTypeLft) ? 2
                                                                                  : 3));
    // Is the right arg RAT, VFP, ARB, or Other?
    uRht = (IsRealRat (aplTypeRht) ? 0
                               : (IsRealVfp (aplTypeRht) ? 2
                                                         : IsRealArb (aplTypeRht) ? 2
                                                                                  : 3));
    // Split cases based upon the left vs. right arg storage types
    switch (4 * uLft + 1 * uRht)
    {
        //      RAT     RAT
        case 4 * 0 + 1 * 0:
        {
        ...
        case 4 * 0 + 1 * 2:
        {
            FINDMAC (preKmpO, aplTypeLft, aplTypeRht, RvZ)
        } // End RAT vs. ARB
        ...
        case 4 * 1 + 1 * 2:
        {
            FINDMAC (preKmpO, aplTypeLft, aplTypeRht, VvZ)
        } // End VFP vs. Oth
        ...
        case 4 * 2 + 1 * 0:
        {
            FINDMAC (preKmpO, aplTypeLft, aplTypeRht, ZvR)
        } // End Oth vs. RAT

        //      ARB     VFP
        case 4 * 2 + 1 * 1:
        {
            FINDMAC (preKmpO, aplTypeLft, aplTypeRht, ZvV)
        } // End Oth vs. VFP

        //      ARB     Oth
        case 4 * 2 + 1 * 3:
        {
            FINDMAC (preKmpO, aplTypeLft, aplTypeRht, ZvO)
        } // End Oth vs. Oth
