...

{
    ...
    //    of the two (Simple Homogeneous < Simple Heterogeneous < Nested < RAT < VFP < ARB),
    ...
    for (uDim = 0; bRet && uDim < (APLINT) aplNELMLft; uDim++)
    {
        ...
        switch (aplTypeCom)
        {
            ...
            case ARRAY_ARB:
                ...
                bRet = CmpCT_B (atLft.aplArb, atRht.aplArb, fQuadCT, EQ);
            ...
            case ARRAY_BA8F:
                // Use the Equal function
                bRet = EqualBAxFvBAxF (&atLft, &atRht, iHCDim, fQuadCT, WFCN);
