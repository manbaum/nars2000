...
#undef  lpHeader
            ...
            switch (aplType)
            {
                ...
                case ARRAY_BA8F:
                    ...
                    for (i = 0; i < iHCDimArr; i++)
                    // If it's a NaN, ...
                    if (arb_nan_p   (&((LPAPLARB) lpMemArr)[i + u * iHCDimArr]))
                        // Save as a NaN
                        arb_set_nan (((LPAPLARB) lpMemRes)++);
                    else
                        // Initialize to 0/0
                        Myarb_init  (((LPAPLARB) lpMemRes)++);
                    // Skip over this one item in the array
                    ((LPAPLARB) lpMemArr) += iHCDimArr;
...
#undef  lpHeader
                ...
                switch (aplType)
                {
                    ...
                    case ARRAY_ARB:
                        ...
                        for (u = 0;
                             u < aplNELM;
                             u++, ((LPAPLARB) lpMemDst)++, ((LPAPLARB) lpMemSrc)++)
                        {
                            // Zap the copied values
                            ZeroMemory (lpMemDst, sizeof (APLARB));

                            // Copy the source value to the destin
                            arb_init_set ((LPAPLARB) lpMemDst, (LPAPLARB) lpMemSrc);
                        } // End FOR
                    ...
                    case ARRAY_BA2F:
                        ...
                        for (u = 0;
                             u < aplNELM;
                             u++, ((LPAPLBA2F) lpMemDst)++, ((LPAPLBA2F) lpMemSrc)++)
                        {
                            // Zap the copied values
                            ZeroMemory (lpMemDst, sizeof (APLBA2F));

                            // Copy the source value to the destin
                            arb2f_init_set ((LPAPLBA2F) lpMemDst, (LPAPLBA2F) lpMemSrc);
                        } // End FOR
                    ...
                    case ARRAY_BA4F:
                        ...
                        for (u = 0;
                             u < aplNELM;
                             u++, ((LPAPLBA4F) lpMemDst)++, ((LPAPLBA4F) lpMemSrc)++)
                        {
                            // Zap the copied values
                            ZeroMemory (lpMemDst, sizeof (APLBA4F));

                            // Copy the source value to the destin
                            arb4f_init_set ((LPAPLBA4F) lpMemDst, (LPAPLBA4F) lpMemSrc);
                        } // End FOR
                    ...
                    case ARRAY_BA8F:
                        ...
                        for (u = 0;
                             u < aplNELM;
                             u++, ((LPAPLBA8F) lpMemDst)++, ((LPAPLBA8F) lpMemSrc)++)
                        {
                            // Zap the copied values
                            ZeroMemory (lpMemDst, sizeof (APLBA8F));

                            // Copy the source value to the destin
                            arb8f_init_set ((LPAPLBA8F) lpMemDst, (LPAPLBA8F) lpMemSrc);
                        } // End FOR
...

{
    ...
    switch (aplType)
    {
        ...
        case ARRAY_ARB:
            return imul64 (sizeof (APLARB)    , aplNELM                     , lpbRet, EXCEPTION_RESULT_FLOAT);
        ...
        case ARRAY_BA2F:
            return imul64 (sizeof (APLBA2F)   , aplNELM                     , lpbRet, EXCEPTION_RESULT_FLOAT);

        case ARRAY_BA4F:
            return imul64 (sizeof (APLBA4F)   , aplNELM                     , lpbRet, EXCEPTION_RESULT_FLOAT);

        case ARRAY_BA8F:
            return imul64 (sizeof (APLBA8F)   , aplNELM                     , lpbRet, EXCEPTION_RESULT_FLOAT);
