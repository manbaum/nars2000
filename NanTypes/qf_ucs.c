...

{
    ...
    if (IsScalar (aplRankRht))
    {
        ...
        switch (aplTypeRht)
        {
            ...
            case ARRAY_ARB:
                aplLongestRht = arb_get_sctsx ((LPAPLARB) VarArrayDataFmBase (lpMemRht), &bRet);
                if (!bRet)
                    goto DOMAIN_EXIT;
                // Check for out of range for Unicode
                //   as an APLUINT so we don't have to deal with negatives
...
#undef  lpAPA
                ...
                switch (GetPtrTypeDir (aplHeteroRht))
                {
                    ...
                    case PTRTYPE_HGLOBAL:
                        ...
                        switch (aplTypeHet)
                        {
                            ...
                            case ARRAY_ARB:
                                aplLongestRht = arb_get_sctsx ((LPAPLARB) lpMemHeteroRht, &bRet);
        ...
        case ARRAY_ARB:
            for (uRht = 0; uRht < aplNELMRht; uRht++)
            {
                ...
                aplLongestRht = arb_get_sctsx (((LPAPLARB) lpMemRht)++, &bRet);
                if (!bRet)
                    goto DOMAIN_EXIT;
