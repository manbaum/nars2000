...
// DecPoint & ExpPoint & RatPoint & ExtPoint & VfpPoint & ArbPoint & Constants
DRVPoint:
    ...
    | ArbPoint                      {DbgMsgWP (L"%%DRVPoint:  ArbPoint");
                                    }
...
EPGArgs:
    ...
    | ArbPoint                      {DbgMsgWP (L"EPGArgs:  ArbPoint");}
    ;
...
// Arb Midpoint args
ArbMid:
      DecPoint                      {DbgMsgWP (L"%%ArbMid:  DecPoint");
                                    }
    | ExpPoint                      {DbgMsgWP (L"%%ArbMid:  ExpPoint");
                                    }
    | RatPoint                      {DbgMsgWP (L"%%ArbMid:  RatPoint");
                                    }
    | ExtPoint                      {DbgMsgWP (L"%%ArbMid:  ExtPoint");
                                    }
    | VfpPoint                      {DbgMsgWP (L"%%ArbMid:  VfpPoint");
                                    }
    | DecConstants                  {DbgMsgWP (L"%%ArbMid:  DecConstants");
                                    }
    | RatConstantsInt               {DbgMsgWP (L"%%ArbMid:  RatConstantsInt");
                                    }
    | RatConstantsExt               {DbgMsgWP (L"%%ArbMid:  RatConstantsExt");
                                    }
    | VfpConstants                  {DbgMsgWP (L"%%ArbMid:  VfpConstants");
                                    }
    ;

// Arb Radius args
ArbRad:
      DecPoint                      {DbgMsgWP (L"%%ArbRad:  DecPoint");
                                    }
    | ExpPoint                      {DbgMsgWP (L"%%ArbRad:  ExpPoint");
                                    }
    | DecConstants                  {DbgMsgWP (L"%%ArbRad:  DecConstants");
                                    }
    ;

ArbPoint:
      ArbMid   DEF_ARBSEP           {DbgMsgWP (L"%%ArbPoint:  ArbMid '±'");
                                     // Make it into a ArbPoint number
                                     lppnLocalVars->lpYYRes = PN_MakeArbPoint   (&$1, NULL, lppnLocalVars);
                                     if (lppnLocalVars->lpYYRes EQ NULL)
                                         YYERROR2;
                                     $$ = *lppnLocalVars->lpYYRes;
                                    }
    | ArbMid   DEF_ARBSEP ArbRad    {DbgMsgWP (L"%%ArbPoint:  ArbMid '±' ArbRad");
                                     // Make it into a ArbPoint number
                                     lppnLocalVars->lpYYRes = PN_MakeArbPoint   (&$1, &$3,  lppnLocalVars);
                                     if (lppnLocalVars->lpYYRes EQ NULL)
                                         YYERROR2;
                                     $$ = *lppnLocalVars->lpYYRes;
                                    }
...

{
    ...
    else
    {
        ...
        if (lpYYLval->chCur EQ 'x'
         && (lppnLocalVars->lpszStart[lppnLocalVars->uNumCur] EQ DEF_ARBSEP
