...

{
    ...
    Assert (IsAPLCharSlope (lptkFunc->tkData.tkChar));
            ...
            for (uHi = 0; uHi < uDimHi; uHi++)
            {
                ...
                for (uAcc = uAx = 0; uAx < aplNELMLft; uAx++)
                {
                    ...
                    if (uLen)
                    {
                        ...
                        // Loop through all of the parts
                        for (i = 0; i < iHCDimRht; i++)
                            arb_init_set (&((LPAPLARB) lpMemRes)[i + iHCDimRht * (uDimRes + uAx * uDimHi)], &atRht.aplBA8F.parts[i]);
                    } else
                        // Loop through all of the parts
                        for (i = 0; i < iHCDimRht; i++)
                            Myarb_init     (&((LPAPLARB) lpMemRes)[i + iHCDimRht * (uDimRes + uAx * uDimHi)]);
                } // End FOR
            } // End FOR/FOR
