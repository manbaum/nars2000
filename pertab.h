...
typedef struct tagPERTABDATA
{
	...
	HGLOBAL 	 hGlbMF_DydIota,	// E0:	Extended dyadic iota
				 hGlbMF_MonUpShoe,	// E4:	Monadic UpShoe
				 hGlbMF_DydTilde;	// E8:	Dyadic tilde

	UINT		 SILevel;			// EC:	Current State Indicator level
	HANDLE		 hSemaDelay;		// F0:	Delay semaphore (NULL if no delay active)
	EXIT_TYPES	 ImmExecExitType;	// F4:	ImmExec exit type (see EXIT_TYPES)
	PL_YYSTYPE	 YYResExec; 		// F8:	Result from execute primitive

	APLCHAR 	 cQuadPR;			//128:	[]PR	 (' ') (When a char scalar)
	APLBOOL 	 bQuadxSA;			//12A:	[]SA (in its index form)
	char		 DPFE[_MAX_PATH];	//12B:	The Drive, Path, Filename, & Ext of the WS
									//22F:	Length
