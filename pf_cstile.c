...

{
    ...
    Assert (lptkFunc->tkData.tkChar EQ UCS2_CIRCLESTILE
         || lptkFunc->tkData.tkChar EQ UCS2_CIRCLEBAR);
...

{
    ...
    } else
    {
        ...
        if (lptkFunc->tkData.tkChar EQ UCS2_CIRCLESTILE)
...

{
    ...
    if (aplRankLft NE 0)
    {
        ...
        for (uDim = 0; uDim < aplRankRht; uDim++)
        if (uDim NE aplAxis
         && lpMemLftDim[uDim] NE lpMemRhtDim[uDim + (uDim < aplAxis)])
        {
...
#undef  lpHeader
    ...
    // Skip over the header to the dimensions
    lpMemRes = VarArrayBaseToDim (lpMemRes);
    lpMemRhtDim = lpMemRht = VarArrayBaseToDim (lpMemRht);
    ...
    else
    {
        ...
        for (uDim = 0, uDimLo = 1; uDim < aplAxis; uDim++)
            uDimLo *= lpMemRhtDim[uDim];

        // Get the length of the axis dimension
        uDimAx = lpMemRhtDim[uDim++];

        // Calculate the product of the right arg's dimensions below the axis dimension
        for (uDimHi = 1; uDim < aplRankRht; uDim++)
            uDimHi *= lpMemRhtDim[uDim];
    } // End IF/ELSE
