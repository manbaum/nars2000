...
UBOOL NfnsArgConv
    (APLSTYPE   aplType,            // Arg storage type
     ...
     LPDR_TYPES lpDiskConv,         // Ptr to result disk conversion
     DR_TYPES   defDiskConv,        // Default disk oncversion
     LPDR_TYPES lpWsConv,           // Ptr to result workspace conversion
     DR_TYPES   defWsConv,          // Default workspace conversion
    ...
    *lpDiskConv = defDiskConv;
    ...
    switch (aplType)
    {
        ...
        case ARRAY_ARB:
            ...
            *lpDiskConv = arb_get_sctsx (&((LPAPLARB) lpMem)[uIndex], &bRet);
                            ...
                            switch (lpMemHdr->NELM)
                            {
                                case 2:
                                    if (bOnly1)
                                        ...
                                        // Attempt to convert the ARB to an integer using System []CT
                                        *lpWsConv   = arb_get_sctsx (&((LPAPLARB) VarArrayDataFmBase (lpMemHdr))[1], &bRet);

                                    // Fall through to singleton case

                                case 1:
                                    // Attempt to convert the ARB to an integer using System []CT
                                    *lpDiskConv = arb_get_sctsx (&((LPAPLARB) VarArrayDataFmBase (lpMemHdr))[0], &bRet);
...
DR_TYPES NestedCharVec
    (HGLOBAL hGlb,                      // Global memory handle
     DR_TYPES drDef)                    // Default conversion value

{
    DR_TYPES          drRet = 0;        // Return value
    UINT              uCnt;             // Loop counter
...
typedef struct tagCONV_CODES
{
    ...
    DR_TYPES   drVal;                   // Matching conversion code
...
UBOOL NfnsSaveData_EM
    (HANDLE       hFile,            // File handle
     ...
     DR_TYPES     DiskConv,         // Disk format
     DR_TYPES     WsConv,           // Workspace format
...
UBOOL NfnsWriteData_EM
    (HANDLE       hFile,                // File handle
     ...
     DR_TYPES     DiskConv,             // Disk conversion value (DR_xxx)
