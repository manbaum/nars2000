...
#undef  lpAPA
    ...
    for (uRes = 0; uRes < aplNELMRes; uRes++)
    {
        ...
        switch (aplTypeRht)
        {
            ...
            case ARRAY_VFP:
                // Copy element # uRht from the right arg to lpMemRes[uRes]
                mpfr_init_copy (&((LPAPLVFP) lpMemRes)[uRes],
            ...
            case ARRAY_HC2R:
                ...
                for (i = 0; i < 2; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    mpq_init_set (&((LPAPLHC2R) lpMemRes)[uRes].parts[i],
                                  &((LPAPLHC2R) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_HC4R:
                ...
                for (i = 0; i < 4; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    mpq_init_set (&((LPAPLHC4R) lpMemRes)[uRes].parts[i],
                                  &((LPAPLHC4R) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_HC8R:
                ...
                for (i = 0; i < 8; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    mpq_init_set (&((LPAPLHC8R) lpMemRes)[uRes].parts[i],
                                  &((LPAPLHC8R) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_HC2V:
                ...
                for (i = 0; i < 2; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    mpfr_init_copy (&((LPAPLHC2V) lpMemRes)[uRes].parts[i],
                                    &((LPAPLHC2V) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_HC4V:
                ...
                for (i = 0; i < 4; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    mpfr_init_copy (&((LPAPLHC4V) lpMemRes)[uRes].parts[i],
                                    &((LPAPLHC4V) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_HC8V:
                ...
                for (i = 0; i < 8; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    mpfr_init_copy (&((LPAPLHC8V) lpMemRes)[uRes].parts[i],
                                    &((LPAPLHC8V) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_BA2F:
                ...
                for (i = 0; i < 2; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    arb_init_set (&((LPAPLBA2F) lpMemRes)[uRes].parts[i],
                                  &((LPAPLBA2F) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_BA4F:
                ...
                for (i = 0; i < 4; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    arb_init_set (&((LPAPLBA4F) lpMemRes)[uRes].parts[i],
                                  &((LPAPLBA4F) lpMemRht)[uRht].parts[i]);
                break;
            ...
            case ARRAY_BA8F:
                ...
                for (i = 0; i < 8; i++)
                    // Copy element # uRht from the right arg to lpMemRes[uRes]
                    arb_init_set (&((LPAPLBA8F) lpMemRes)[uRes].parts[i],
                                  &((LPAPLBA8F) lpMemRht)[uRht].parts[i]);
                break;
...
HGLOBAL PrimFnDydUpDownArrowLftGlbValid_EM
    (LPAPLNELM         lpaplNELMCom,    // Ptr to common NELM (may be NULL if DownArrow)
     ...
     LPVARARRAY_HEADER lpMemHdrLft,     // Ptr to left arg header (may be NULL if immediate)
     APLSTYPE          aplTypeLft,      // Left arg storage type
     APLLONGEST        aplLongestLft,   // Left arg immediate value
...
#undef  lpHeader
        ...
        lpDataLft = VarArrayDataFmBase (lpMemHdrLft);
    } else
    {
        aplNELMLft = 1;
        aplRankLft = 0;
...
#undef  lpAPA
        ...
        {
            ALLTYPES at = {0};
            at.enumCT = ENUMCT_SYS;     // Use SYS_CT when comparing against zero

            for (uDim = 0; bRet && uDim < aplNELMLft; uDim++)
            {
                // Attempt to convert the ARB to an integer using System []CT
                aplIntLft = ConvertArbToInt (&((LPAPLARB) lpDataLft)[uDim], 0, &at, &bRet);
                if (bRet)
                {
                    // If there's an axis, ...
                    if (lpMemAxisTail NE NULL)
    ...
    if (bRet)
    {
        ...
        *lpaplNELMRes = 1;
        ...
        for (uDim = 0; bRet && uDim < aplRankRes; uDim++)
        {
            ...
            aplIntTmp = imul64 (*lpaplNELMRes, aplIntLft, &bRet, EXCEPTION_RESULT_FLOAT);
