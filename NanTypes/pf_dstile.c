...
PRIMSPEC PrimSpecDownStile = {
    ...
    &PrimFnMonDownStileBA1FisBA1F,
    &PrimFnMonDownStileBA2FisBA2F,
    &PrimFnMonDownStileBA4FisBA4F,
    NULL,   // &PrimFnMonDownStileBA8FisBA8F, -- Can't happen w/DownStile
    &PrimFnMonDownStileBA1FisBA1F,
    NULL,   // &PrimFnMonDownStileARBisBA2F, -- Can't happen w/DownStile
    NULL,   // &PrimFnMonDownStileARBisBA4F, -- Can't happen w/DownStile
    NULL,   // &PrimFnMonDownStileARBisBA8F, -- Can't happen w/DownStile
    ...
    NULL,   // &PrimFnDydDownStileBisBA1FvBA1F, -- Can't happen w/DownStile
    NULL,   // &PrimFnDydDownStileBisBA2FvBA2F, -- Can't happen w/DownStile
    NULL,   // &PrimFnDydDownStileBisBA4FvBA4F, -- Can't happen w/DownStile
    NULL,   // &PrimFnDydDownStileBisBA8FvBA8F, -- Can't happen w/DownStile
    ...
    &PrimFnDydDownStileBA1FisBA1FvBA1F,
    NULL,   // &PrimFnDydDownStileBA2FisBA2FvBA2F, -- Can't happen w/DownStile
    NULL,   // &PrimFnDydDownStileBA4FisBA4FvBA4F, -- Can't happen w/DownStile
    NULL,   // &PrimFnDydDownStileBA8FisBA8FvBA8F, -- Can't happen w/DownStile
...
////    WCHAR wszTemp[512];
        ...
        // Split cases based upon the signum of the difference between
        //   (the number and its floor) and (the ceiling and the number)
        switch (signumint (arb_cmp (&arbTmp1, &arbTmp2)))
        {
            case  1:
                arb_set (&arbNear, &arbCeil);

                break;

            case  0:
                arb_abs2 (&arbTmp1, &arbFloor);
                arb_abs2 (&arbTmp2, &arbFloor);
...
////    strcpyW (wszTemp, L"Ceil:  "); *FormatAplArb (&wszTemp[lstrlenW (wszTemp)], arbCeil , 0) = WC_EOS; DbgMsgW (wszTemp);
        ...
        // If Near is < Rht, aplRes = Near
        if (arb_cmp (&arbNear, &aplRht) < 0)
            arb_set (&aplRes, &arbNear);
        else
        {
            // If Near is non-zero, and
            //   Rht is tolerantly-equal to Near,
            //   aplRes = Near; otherwise, aplRes = Near - 1
            if (arb_sign (&arbNear) NE 0)
            {
                arb_set (&aplRes, &arbNear);
                ...
                if (CmpSCT_B (aplRht,
                             ...
                             GetQuadCT (),
                             NE))
                    arb_sub_ui (&aplRes, &aplRes, 1, prec);
            } else
            {
                // aplNear is zero

                // Get -[]CT as an ARB
                arb_set_d (&arbTmp1, -GetQuadCT ());

                // If Rht is between (-[]CT) and 0 (inclusive),
                //   aplRes = 0; otherwise, aplRes = -1
                if (arb_cmp (&arbTmp1, &aplRht) <= 0
                 && arb_sign (&aplRht)          <= 0)
                    arb_set_si (&aplRes,  0);
                else
                    arb_set_si (&aplRes, -1);
...
//  $PrimFnMonDownStileBA1FisBA1F
//
//  Primitive scalar function monadic DownStile:  BA1F {is} fn BA1F
//***************************************************************************

void PrimFnMonDownStileBA1FisBA1F
...
{
    // Save in the result
    lpMemRes[uRes] = FloorBA1F (lpatRht->aplArb);
} // End PrimFnMonDownStileBA1FisBA1F
...

{
    ...
    APLBA1F  aplSum = {0},
             aplArbFlr = {0};
    APLBA1F  aplDst1 = {0},             // Distance between aplRht and aplTmp1
             aplDst2 = {0};             // ...                         aplTmp2
    APLINT   aplIntFlr;
    mp_limb_signed_t prec = ARB_PREC_FPC;
    ...
    if (gUseHurwitz)
    {
        ...
        // Initialize the half integer coordinate choice to 0/0
        arb2f_init (&aplTmp2);
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            // Add 0.5 to each part
            aplSum = AddBA1F_RE (aplRht.parts[i], arbHalf);

            // Calculate the floor of each part
            aplTmp1.parts[i] = FloorBA1F (aplSum);
            ...
            // Double it
            arb_mul_si (&aplTmp2.parts[i], &aplRht.parts[i], 2, prec);

            // Add 0.5 to each part
            aplSum = AddBA1F_RE (aplTmp2.parts[i], arbHalf);

            // Calculate the floor of each part
            aplArbFlr = FloorBA1F (aplSum);

            Assert (arb_integer_p (&aplArbFlr));

            // We no longer need this storage
            Myarb_clear (&aplSum);

            // Extract the integer within
            aplIntFlr = ConvertToInteger_SCT (ARRAY_BA1F, &aplArbFlr, 0, &bRet);

            Assert (bRet);

            // We no longer need this storage
            Myarb_clear (&aplArbFlr);
            ...
            // Save half the value as a RAT
            mpq_set_sx (&aplRatTmp, aplIntFlr, 2);

            // Save half the value as an ARB
            arb_set_q (&aplTmp2.parts[i], &aplRatTmp);
        } // End FOR
        ...
        // Calculate the distances
        aplDst1 = DistBA2F (aplRht, aplTmp1);
        aplDst2 = DistBA2F (aplRht, aplTmp2);

        // Choose the smaller distance
        //   with ties going to the integer coordinates
        if (CmpSCT_B (aplDst1, aplDst2, SYS_CT, <=))
        {
            // Save integer coordinates in the result
            arb2f_init_set (&aplFlr, &aplTmp1);
    ...
    } else
    {
        ...
        {
            // Calculate the floor of each part
            aplFlr.parts[i] = FloorBA1F (aplRht.parts[i]);

            // Calculate the fractional value of each part
            aplFrc.parts[i] = ModBA1F (arbOne, aplRht.parts[i]);

            // Calculate the sum of the fractional values
            arb_add (&aplSum, &aplSum, &aplFrc.parts[i], prec);
        } // End FOR

        // Compare against the hyperplane between the nearest corners
        bRet = CmpSCT_B (aplSum, arbOne, 0, >=);
        ...
        if (bRet)
        {
            ...
            for (i = 1; i < 2; i++)
            // If this part is larger (within []CT), ...
            if (CmpSCT_B (aplSum, aplFrc.parts[i], SYS_CT, <))
            {
                // Save as the largest so far
                j = i; aplSum = aplFrc.parts[j];
            } // End FOR/IF
            ...
            // Increment the chosen coordinate
            arb_add (&aplFlr.parts[j], &aplFlr.parts[j], &arbOne, prec);
        } // End IF
...

{
    ...
    // Calculate the distances
    aplDst1 = DistBA4F (aplRht, aplTmp1);
    aplDst2 = DistBA4F (aplRht, aplTmp2);
...
//  $PrimFnMonDownStileBA2FisBA2F
//
//  Primitive scalar function monadic DownStile:  BA2F {is} fn BA2F
//***************************************************************************

void PrimFnMonDownStileBA2FisBA2F
...
{
    // Save in the result
    lpMemRes[uRes] = FloorBA2F (lpatRht->aplBA2F);
} // End PrimFnMonDownStileBA2FisBA2F


//***************************************************************************
//  $PrimFnMonDownStileBA4FisBA4F
//
//  Primitive scalar function monadic DownStile:  BA4F {is} fn BA4F
//***************************************************************************

void PrimFnMonDownStileBA4FisBA4F
...
{
    // Save in the result
    lpMemRes[uRes] = FloorBA4F (lpatRht->aplBA4F);
} // End PrimFnMonDownStileBA4FisBA4F
...
//  $PrimFnDydDownStileBA1FisBA1FvBA1F
//
//  Primitive scalar function dyadic DownStile:  BA1F {is} BA1F fn BA1F
//***************************************************************************

void PrimFnDydDownStileBA1FisBA1FvBA1F
    (LPAPLARB   lpMemRes,           // Ptr to the result
     APLUINT    uRes,               // Index into the result
     LPALLTYPES lpatLft,            // Ptr to left arg ALLTYPES
     LPALLTYPES lpatRht,            // ...    right ...
...

{
    ...
    arb_min (&lpMemRes[uRes], &lpatLft->aplArb, &lpatRht->aplArb, prec);
} // End PrimFnDydDownStileBA1FisBA1FvBA1F
