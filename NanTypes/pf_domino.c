...
////////lpYYRes->tkToken.tkData.tkFloat    =            // Filled in below
        ...
        switch (aplTypeRht)
        {
            ...
            case ARRAY_BA8F:
                ...
                // If the right arg is zero, the result is zero
                //   as per SVD
                if (IsZeroBAxF (lpMemRht, iHCDimRht))
                    // Make a copy of the right arg so we can copy it to the result
                    (*aTypeActPromote[aplTypeRes][aplTypeRes]) (lpMemRht, 0, &atRes);
                else
                // Split cases based upon the storage type
                switch (aplTypeRht)
                {
                    case ARRAY_ARB:
                        // Invert it
                        PrimFnMonColonBarBA1FisBA1F (&atRes.aplArb , 0, lpMemRht, NULL);

                        break;

                    case ARRAY_BA2F:
                        // Invert it
                        PrimFnMonColonBarBA2FisBA2F (&atRes.aplBA2F, 0, lpMemRht, NULL);

                        break;

                    case ARRAY_BA4F:
                        // Invert it
                        PrimFnMonColonBarBA4FisBA4F (&atRes.aplBA4F, 0, lpMemRht, NULL);

                        break;

                    case ARRAY_BA8F:
                        // Invert it
                        PrimFnMonColonBarBA8FisBA8F (&atRes.aplBA8F, 0, lpMemRht, NULL);
    ...
    switch (aplTypeRes)
    {
        ...
        case ARRAY_ARB:
            ...
            if (uNumRows EQ uNumCols)
            {
                ...
                Assert (uNumRows EQ (slong) uNumRows);
                Assert (uNumCols EQ (slong) uNumCols);

                // Initialize
                arb_mat_init (A, (slong) uNumRows, (slong) uNumCols);
                arb_mat_init (X, (slong) uNumRows, (slong) uNumCols);

                // Loop through the rows & cols of the right arg
                for (uItm = uRow = 0; uRow < uNumRows; uRow++)
                for (       uCol = 0; uCol < uNumCols; uCol++)
                    // Copy the right arg to the matrix A
                    arb_set (arb_mat_entry (A, uRow, uCol), &((LPAPLARB) lpMemRht)[uItm++]);

                // Invert the matrix
                iRet = arb_mat_inv (X, A, prec);

                // Loop through the rows & cols of the right arg
                for (uRow = 0; uRow < uNumRows; uRow++)
                for (uCol = 0; uCol < uNumCols; uCol++)
                    // Copy X to lpMemRes
                    arb_init_set (((LPAPLARB) lpMemRes)++, arb_mat_entry (X, uRow, uCol));
        ...
        case ARRAY_BA2F:
            ...
            if (uNumRows EQ uNumCols)
            {
                ...
                // Initialize
                acb_mat_init (A, (slong) uNumRows, (slong) uNumCols);
                acb_mat_init (X, (slong) uNumRows, (slong) uNumCols);
                ...
                {
                    // Initialize
                    acb_init ((acb_struct *)              lpMemRes);

                    // Copy X to lpMemRes
                    acb_set  ((acb_struct *) ((LPAPLBA2F) lpMemRes)++, acb_mat_entry (X, uRow, uCol));
                } // End FOR/FOR
        ...
        case ARRAY_BA8F:
            ...
            for (uRow = 0; uRow < (aplNELMRht * iHCDimRht); uRow++)
                // Copy and convert the entry
                arb_init_set (&((LPAPLARB) lpMemRes)[uRow],
                              &((LPAPLARB) lpMemRht)[uRow]);

            // Use Gauss-Jordan elimination to invert the matrix in place
            if (!GaussJordanBAxF (lpMemRes, uNumRows, uNumCols, NULL, 0, iHCDimRht, lpbCtrlBreak))
                goto DOMAIN_EXIT;
...
////////////////
        ...
        case ARRAY_ARB:
            // If the right arg matrix is Square, ...
            if (uNumRowsRht EQ uNumColsRht)
            {
                APLDIM    uItm;         // Loop counter
                arb_mat_t A,            // The right arg input
                          B,            // ... left ...
                          X;            // ... output
                mp_limb_signed_t prec = ARB_PREC_FPC;
                int       iRet;

                Assert (uNumRowsLft EQ (slong) uNumRowsLft);
                Assert (uNumColsLft EQ (slong) uNumColsLft);

                Assert (uNumRowsRht EQ (slong) uNumRowsRht);
                Assert (uNumColsRht EQ (slong) uNumColsRht);

                // Initialize
                arb_mat_init (A, (slong) uNumRowsRht, (slong) uNumColsRht);
                arb_mat_init (B, (slong) uNumRowsLft, (slong) uNumColsLft);
                arb_mat_init (X, (slong) uNumRowsLft, (slong) uNumColsLft);

                // Loop through the rows & cols of the right arg
                for (uItm = uRow = 0; uRow < uNumRowsRht; uRow++)
                for (       uCol = 0; uCol < uNumColsRht; uCol++)
                {
                    // Promote the right arg to ARB
                    (*aTypeActPromote[aplTypeRht][ARRAY_ARB]) (lpMemRht, uItm++, &atTmp);

                    // Copy the right arg to the matrix A
                    arb_set (arb_mat_entry (A, uRow, uCol), &atTmp.aplArb);

                    // Free the temp
                    (*aTypeFree[ARRAY_ARB]) (&atTmp, 0);
                } // End FOR/FOR

                // Loop through the rows & cols of the left arg
                for (uItm = uRow = 0; uRow < uNumRowsLft; uRow++)
                for (       uCol = 0; uCol < uNumColsLft; uCol++)
                {
                    // Promote the left arg to ARB
                    (*aTypeActPromote[aplTypeLft][ARRAY_ARB]) (lpMemLft, uItm++, &atTmp);

                    // Copy the right arg to the matrix B
                    arb_set (arb_mat_entry (B, uRow, uCol), &atTmp.aplArb);
                ...
                for (uCol = 0; uCol < uNumColsLft; uCol++)
                {
                    ...
                    // Copy X to lpMemRes
                    arb_set  (((LPAPLARB) lpMemRes)++, arb_mat_entry (X, uRow, uCol));
                } // End FOR/FOR
        ...
        case ARRAY_BA2F:
            ...
            {
                APLDIM    uItm;         // Loop counter
                acb_mat_t A,            // The right arg input
                          B,            // ... left ...
                          X;            // ... output
                mp_limb_signed_t prec = ARB_PREC_FPC;
                int       iRet;
                ...
                // Initialize
                acb_mat_init (A, (slong) uNumRowsRht, (slong) uNumColsRht);
                acb_mat_init (B, (slong) uNumRowsLft, (slong) uNumColsLft);
                acb_mat_init (X, (slong) uNumRowsLft, (slong) uNumColsLft);

                // Loop through the rows & cols of the right arg
                for (uItm = uRow = 0; uRow < uNumRowsRht; uRow++)
                for (       uCol = 0; uCol < uNumColsRht; uCol++)
                {
                    // Promote the right arg to BA2F
                    (*aTypeActPromote[aplTypeRht][ARRAY_BA2F]) (lpMemRht, uItm++, &atTmp);

                    // Copy the right arg to the matrix A
                    acb_set (acb_mat_entry (A, uRow, uCol), (const acb_struct *) &atTmp.aplArb);
                ...
                for (       uCol = 0; uCol < uNumColsLft; uCol++)
                {
                    // Promote the left arg to BA2F
                    (*aTypeActPromote[aplTypeLft][ARRAY_BA2F]) (lpMemLft, uItm++, &atTmp);

                    // Copy the right arg to the matrix B
                    acb_set (acb_mat_entry (B, uRow, uCol), (const acb_struct *) &atTmp.aplArb);

                    // Free the temp
                    (*aTypeFree[ARRAY_BA2F]) (&atTmp, 0);
...
////////case ARRAY_ARB:         // These cases fall through to here for non-square right arg matrices
////////case ARRAY_BA2F:        // ...
        case ARRAY_BA4F:
            // Create the temporary left and right matrices
            hGlbTmpLft = DbgGlobalAlloc (GHND, (APLU3264) (uNumRowsLft * uNumColsLft * iSizeofRes));
            hGlbTmpRht = DbgGlobalAlloc (GHND, (APLU3264) (uNumRowsRht * uNumColsRht * iSizeofRes));
            ...
            {
                // Promote the entry
                (*aTypeActPromote[aplTypeRht][aplTypeRes]) (lpMemRht, uRow, &atTmp);

                // Save in the result (to be used as the right arg to <GaussJordanBAxF>
                ((LPAPLARB) lpMemTmpRht)[uRow] = atTmp.aplArb;

                // Zero the memory in case we might free it
                ZeroMemory (&atTmp, sizeof (atTmp));
            } // End FOR

            // Loop through the items of the left arg
            for (uRow = 0; uRow < (aplNELMLft * iHCDimLft); uRow++)
            {
                // Promote the entry
                (*aTypeActPromote[aplTypeLft][aplTypeRes]) (lpMemLft, uRow, &atTmp);

                // Save in the result (to be used as the left arg to GaussJordan
                ((LPAPLARB) lpMemTmpLft)[uRow] = atTmp.aplArb;
            ...
            // Use Gauss-Jordan elimination to invert the matrix in place
            bRet = GaussJordanBAxF (lpMemTmpRht, uNumRowsRht, uNumColsRht, lpMemTmpLft, uNumColsLft, iHCDimRes, lpbCtrlBreak);
            ...
            if (bRet)
            {
                ...
                // Loop through the items of the result
                for (uRow = 0; uRow < (aplNELMRes * iHCDimRes); uRow++)
                    // Save in the result
                    arb_init_set (((LPAPLARB) lpMemRes)++, ((LPAPLARB) lpMemTmpLft)++);
            } else
                goto DOMAIN_EXIT;
            break;
...
UBOOL GaussJordanBAxF
    (LPVOID   lpMemRht2,            // Ptr to right arg/result as APLARB matrix
     APLDIM   uNumRows,             // # rows in the left/right arg/result
     APLDIM   uNumColsRht,          // # cols ...
     LPVOID   lpMemLft2,            // Ptr to left arg as APLARB matrix (may be NULL)
     APLDIM   uNumColsLft,          // # cols in the left arg
     int      iHCDimRht,            // Right arg HC Dimension (1, 2, 4, 8)
     LPUBOOL  lpbCtrlBreak)         // Ptr to Ctrl-Break flag

{
    ...
    {
        case 1:
            PF_DOMINO_ARB (((LPAPLBA1F) lpMemRht2), ((LPAPLBA1F) lpMemLft2), 1);

        case 2:
            PF_DOMINO_ARB (((LPAPLBA2F) lpMemRht2), ((LPAPLBA2F) lpMemLft2), 2);

        case 4:
            PF_DOMINO_ARB (((LPAPLBA4F) lpMemRht2), ((LPAPLBA4F) lpMemLft2), 4);

        case 8:
            PF_DOMINO_ARB (((LPAPLBA8F) lpMemRht2), ((LPAPLBA8F) lpMemLft2), 8);
