...
////lpYYRes->tkToken.tkData.tkInteger  =   (filled in below)
    ...
    // Set the datatype
    lpYYRes->tkToken.tkData.tkInteger = TranslateArrayTypeToDrType (aplTypeRht);
...

{
    ...
    LPVOID            lpMemTmp;
    LPAPLVFP          lpMemVfpRht;          // Ptr to VFP global memory data
    LPAPLARB          lpMemArbRht;          // ...    ARB ...
    LPVOID            lpMemRht;             // ...    NESTED/HETERO ...
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_HC8V:
            ...
            lpMemVfpRht = VarArrayDataFmBase (lpMemHdrRht);
            ...
            {
                // Get the precision
                aplPrec = mpfr_get_prec (&lpMemVfpRht[uCnt]);
        ...
        case ARRAY_BA8F:
            ...
            // Skip over the header and dimensions to the data
            lpMemArbRht = VarArrayDataFmBase (lpMemHdrRht);
            ...
            {
                // Get the precision
                aplPrec = arb_bits (&lpMemArbRht[uCnt]);
...
////////////////////////////
                        ...
                        case ARRAY_BA8F:
                            ...
                            {
                                // Get the precision
                                aplPrec = arb_bits (&((LPAPLARB) lpMemTmp)[i]);
...
#undef  lpHeader
    ...
    {
        // Save the last dimension
        *VarArrayBaseToDim (lpMemHdrRes) = aplColsRes;
...

{
    ...
    HGLOBAL           hGlbRht,              // Right arg global memory handle
                      hGlbRes;              // Result    ...
    LPVARARRAY_HEADER lpMemHdrRht = NULL,   // Ptr to right arg array header
                      lpMemHdrRes = NULL;   // ...    result    ...
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_ARB:
            MySprintfW (wszTemp,
                        sizeof (wszTemp),
                       L"Real Ball Arithmetic (%d):  variable precision midpoint, 64-bit radius",
                        DR_ARB);
            break;
        ...
        case ARRAY_BA2F:
            MySprintfW (wszTemp,
                        sizeof (wszTemp),
                       L"Complex Ball Arithmetic (%d):  variable precision midpoint, 64-bit radius",
                        DR_BA2F);
            break;
        ...
        case ARRAY_BA4F:
            MySprintfW (wszTemp,
                        sizeof (wszTemp),
                       L"Quaternion Ball Arithmetic (%d):  variable precision midpoint, 64-bit radius",
                        DR_BA4F);
            break;
        ...
        case ARRAY_BA8F:
            MySprintfW (wszTemp,
                        sizeof (wszTemp),
                       L"Octonion Ball Arithmetic (%d):  variable precision midpoint, 64-bit radius",
                        DR_BA8F);
            break;
     ...
     && !IsEmpty (aplNELMRht))
    {
        ...
        // Skip over the header and dimensions to the data
        lpaplVfp = VarArrayDataFmBase (lpMemHdrRht);
    ...
    // If the array is any kind of ARB, ...
    //   and it's non-empty, ...
    if (IsRealArb (aplTypeRht)
     && !IsEmpty (aplNELMRht))
    {
        LPAPLARB lpaplArb;          // Ptr to ARB value
        APLUINT  uCommPrec;         // Ptr to common ARB array precision (0 if none) (may be NULL)
        ...
        // Skip over the header and dimensions to the data
        lpaplArb = VarArrayDataFmBase (lpMemHdrRht);

        // Get the initial precision
        uCommPrec = arb_bits (lpaplArb++);

        // If the Midpoint is a special value (NaN, Inf, ...), ...
        if (uCommPrec EQ 0)
            // Mark as Special
            MyStrcatW (wszTemp, sizeof (wszTemp), L" " AP_FPC L"-Special");
        else
        {
            for (uRht = 1; uRht < uLen; uRht++)
            if (uCommPrec NE arb_bits (lpaplArb++))
            {
                // Mark as none
                uCommPrec = 0;
...

{
    ...
    LPAPLDIM          lpMemDimRht,          // Ptr to right arg dimensions
                      lpMemDimRes;          // ...    result    ...
    LPVOID            lpMemRht;             // Ptr to right arg global memory
    LPAPLCHAR         lpMemRes;             // Ptr to result    ...
...
#undef  lpHeader
    ...
    *lpMemDimRes++ = 16;
    ...
    if (lpMemHdrRht NE NULL)
    {
        ...
        for (uRes = 0; bRet && uRes < aplNELMRht; uRes++, lpMemRes += 16)
        {
...
#undef  lpHeader
    ...
    Assert (16 EQ *lpMemDimRht); lpMemDimRht++;
