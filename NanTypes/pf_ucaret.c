...
PRIMSPEC PrimSpecUpCaret = {
    ...
    NULL,   // &PrimFnMonUpCaretBA1FisBA1F,
    NULL,   // &PrimFnMonUpCaretBA2FisBA2F,
    NULL,   // &PrimFnMonUpCaretBA4FisBA4F,
    NULL,   // &PrimFnMonUpCaretBA8FisBA8F,
    NULL,   // &PrimFnMonUpCaretARBisBA1F,
    NULL,   // &PrimFnMonUpCaretARBisBA2F,
    NULL,   // &PrimFnMonUpCaretARBisBA4F,
    NULL,   // &PrimFnMonUpCaretARBisBA8F,
    ...
    NULL,   // &PrimFnDydUpCaretBisBA1FvBA1F, -- Can't happen w/UpCaret
    NULL,   // &PrimFnDydUpCaretBisBA2FvBA2F, -- Can't happen w/UpCaret
    NULL,   // &PrimFnDydUpCaretBisBA4FvBA4F, -- Can't happen w/UpCaret
    NULL,   // &PrimFnDydUpCaretBisBA8FvBA8F, -- Can't happen w/UpCaret
    ...
    &PrimFnDydUpCaretBA1FisBA1FvBA1F,
    &PrimFnDydUpCaretBA2FisBA2FvBA2F,
    &PrimFnDydUpCaretBA4FisBA4FvBA4F,
    NULL,   // &PrimFnDydUpCaretBA8FisBA8FvBA8F, -- Can't happen w/UpCaret
...
//  $LcmBA1F
//
//  LCM (Least Common Multiple) for aplArbs
//***************************************************************************
...

{
    ...
    // Calculate the GCD
    aplGcd = GcdBA1F (aplLft, aplRht);

    // Calculate the magnitude
    aplMag = MagArb (aplGcd);

    if (arb_cmp_d (&aplMag, GCD_CT) >= 0)
    {
////////aplTmp = (aplLft * aplRht) / aplGcd;

        arb_mul (&aplRes, &aplLft, &aplRht, prec);
        arb_div (&aplRes, &aplRes, &aplGcd, prec);
    } else
        arb_set (&aplRes, &aplGcd);

    // We no longer need this storage
    Myarb_clear (&aplMag);
    Myarb_clear (&aplGcd);
////arb_clear (&aplRes);        // Returned as result
...
//  $LcmBA2F
//
//  LCM (Least Common Multiple) for aplBA2F
//***************************************************************************
...

{
    ...
    {
        // Calculate the GCD
        aplGcd = GcdBA2F (aplLft, aplRht);

        // Calculate the magnitude
        aplMag = MagBA2F (aplGcd);

        if (arb_cmp_d (&aplMag, GCD_CT) >= 0)
        {
            aplMul = MulBA2F_RE (aplLft, aplRht);
            aplRes = DivBA2F_RE (aplMul, aplGcd);
        ...
        // We no longer need this resource
        Myarb_clear   (&aplMag);
////////arb2f_clear (&aplRes);      // Returned as result
    } __except (CheckHCDim (GetExceptionInformation (), 2))
    {
        // We no longer need these resources
        Myarb_clear   (&aplMag);
...
//  $LcmBA4F
//
//  LCM (Least Common Multiple) for aplBA4F
//***************************************************************************
...

{
    ...
    {
        // Calculate the GCD
        aplGcd = GcdBA4F (aplLft, aplRht);

        // Calculate the magnitude
        aplMag = MagBA4F (aplGcd);

        if (arb_cmp_d (&aplMag, GCD_CT) >= 0)
        {
            aplMul = MulBA4F_RE (aplLft, aplRht);
            aplRes = DivBA4F_RE (aplMul, aplGcd);
        ...
        // We no longer need this resource
        Myarb_clear   (&aplMag);
////////arb4f_clear (&aplRes);      // Returned as result
    } __except (CheckHCDim (GetExceptionInformation (), 4))
    {
        // We no longer need these resources
        Myarb_clear   (&aplMag);
...
//  $PrimFnDydUpCaretBA1FisBA1FvBA1F
//
//  Primitive scalar function dyadic UpCaret:  BA1F {is} fn BA1F fn BA1F
//***************************************************************************

void PrimFnDydUpCaretBA1FisBA1FvBA1F
...

{
    ...
    if (arb_inf_p (&lpatLft->aplArb)
     || arb_inf_p (&lpatRht->aplArb))
        ...
        lpMemRes[uRes] = LcmBA1F (lpatLft->aplArb, lpatRht->aplArb);
} // End PrimFnDydUpCaretBA1FisBA1FvBA1F
...
//  $PrimFnDydUpCaretBA2FisBA2FvBA2F
//
//  Primitive scalar function dyadic UpCaret:  BA2F {is} BA2F fn BA2F
//***************************************************************************

void PrimFnDydUpCaretBA2FisBA2FvBA2F
...
{
    lpMemRes[uRes] = LcmBA2F (lpatLft->aplBA2F, lpatRht->aplBA2F);
} // End PrimFnDydUpCaretBA2FisBA2FvBA2F
...
//  $PrimFnDydUpCaretBA4FisBA4FvBA4F
//
//  Primitive scalar function dyadic UpCaret:  BA4F {is} BA4F fn BA4F
//***************************************************************************

void PrimFnDydUpCaretBA4FisBA4FvBA4F
...
{
    lpMemRes[uRes] = LcmBA4F (lpatLft->aplBA4F, lpatRht->aplBA4F);
} // End PrimFnDydUpCaretBA4FisBA4FvBA4F
