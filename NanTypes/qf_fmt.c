                        ...
                        switch (fmtSpecRes)
                        {
                            ...
                            case FMTSPECVAL_V:
                                ...
                                for (uRow = 0; uRow < aplRowsItm; uRow++)
                                    ...
                                    QFMT_CommonEFIRV (*lplpfsNxt,                                       // Ptr to FMTSPECSTR
                                                       aplTypeItm,                                      // Item storage type
                                                       aplNELMItm,                                      // Item NELM
                                                       fmtSpecRes,                                      // Result format spec
                                                       lpMemItm,                                        // Ptr to item global memory
...
//  $QFMT_CommonEFIRV
//
//  Code common to E- F- I- R- and V-format specs
//***************************************************************************

void QFMT_CommonEFIRV
    (LPFMTSPECSTR lpfsNxt,                  // Ptr to FMTSPECSTR
     APLSTYPE     aplTypeItm,               // Item storage type
     APLNELM      aplNELMItm,               // Item NELM
     FMTSPECVAL   fmtSpecRes,               // Result format spec
...

{
    ...
    APLARB       aplArbItm = {0},           // Item as ARB
                 aplArb10 = {0},            // Constant ARB
                 aplArbTmp = {0},           // Temporary ARB
                 aplArbTmp2 = {0};          // ...
    ...
    if (IsImmGlbNum (immTypeItm))
    {
        ...
        if (IsImmArb (immTypeItm))
            arb_init_set_sx (&aplArb10, 10);
        else
            DbgStop ();
    } // End IF
    ...
    switch (immTypeItm)
    {
        ...
        case IMMTYPE_ARB:
            arb_init_set (&aplArbItm, (LPAPLARB) lpSymGlbItm);
            aplFltItm = arb_get_d (&aplArbItm);
            aplIntItm = (APLINT) aplFltItm;
    ...
    switch (immTypeItm)
    {
        ...
        case IMMTYPE_ARB:
            if (arb_cmp_ui (&aplArbItm, 0) EQ 0)
                // Move to the next item
                goto NORMAL_EXIT;
            break;
            ...
            case FMTSPECVAL_V:
                ...
                if (fsScl < 0)
                {
                    ...
                    if (IsImmGlbNum (immTypeItm))
                    {
                        ...
                        if (IsImmArb (immTypeItm))
                            arb_div  (&aplArbItm, &aplArbItm, &aplArb10, prec);
                        else
                            DbgStop ();
                    } else
                        while (fsScl++)
                ...
                } else
                {
                    ...
                    if (IsImmGlbNum (immTypeItm))
                    {
                        ...
                        if (IsImmArb (immTypeItm))
                            arb_mul (&aplArbItm, &aplArbItm, &aplArb10, prec);
                        else
                            DbgStop ();
                    } else
                        while (fsScl--)
    ...
    switch (fmtSpecRes)
    {
        case FMTSPECVAL_E:
        {
            ...
            switch (immTypeItm)
            {
                ...
                case IMMTYPE_ARB:
                    // Izit an infinity?
                    bInfinity = IsArbInfinity (&aplArbItm);

                    lpwEnd =
                      FormatAplArbFC (lpwszFormat,          // Ptr to output save area
                                     &aplArbItm,            // Ptr to the value to format
                            -(APLINT) fsDig,                // # significant digits (0 = all)
                                      0,                    // Maximum width including sign & decpt (0 = none)
                                      UTF16_DOT,            // Char to use as decimal separator
                                      UTF16_OVERBAR,        // Char to use as overbar
        ...
        case FMTSPECVAL_V:
            ...
            switch (immTypeItm)
            {
                case IMMTYPE_BOOL:
                case IMMTYPE_INT:
                    arb_init_set_sx   (&aplArbTmp, aplIntItm);

                    break;

                case IMMTYPE_FLOAT:
                    arb_init_set_d    (&aplArbTmp, aplFltItm);

                    break;

                case IMMTYPE_RAT:
                    arb_init_set_q    (&aplArbTmp, &aplRatItm);

                    break;

                case IMMTYPE_VFP:
                    arb_init_set_mpfr (&aplArbTmp, &aplVfpItm);

                    break;

                case IMMTYPE_ARB:
                    arb_set           (&aplArbTmp, &aplArbItm);
            ...
            // The # fractional digits is fsDig + 1 unless the RAT is an integer, or is between -1 and 1
            iFrcDig = (arb_integer_p (&aplArbItm) || (arb_cmp_si (&aplArbItm, -1) >= 0 && arb_cmp_si (&aplArbItm, 1) <= 0)) ? fsDig : fsDig + 1;

            lpwEnd =
              FormatAplArbFC (lpwszFormat,          // Ptr to output save area
                             &aplArbTmp,            // Ptr to the value to format
                              fsDig,                // # fractional digits (0 = all)
                              fsWid,                // Maximum width including sign & decpt (0 = none)
                              UTF16_DOT,            // Char to use as rational separator
                              UTF16_OVERBAR,        // Char to use as overbar
                              FLTDISPFMT_F,         // Float display format
                              TRUE,                 // TRUE iff nDigits is # fractional digits
                              FALSE,                // TRUE iff we're to substitute text for infinity
            ...
            *--lpwEnd = WC_EOS;
            ...
            // Get ptr to ARB point
            lpwSrc = strchrW (lpwszFormat, DEF_ARBSEP);
        ...
        case FMTSPECVAL_I:
            ...
            switch (immTypeItm)
            {
                ...
                case IMMTYPE_ARB:
                    ...
                    Myarb_init (&aplArbTmp2);

                    // Round the ARB to the nearest integer
                    arb_add   (&aplArbTmp2, &aplArbItm, &arbHalf, prec);
                    aplArbTmp = FloorBA1F (aplArbTmp2);
                    Myarb_clear (&aplArbTmp2);
    ...
    switch (*lpwSymChar)
    {
        ...
        case DEF_ARBSEP:
            *lpwSymChar++ = lpwSub[SYMSUB_ARB_SEP];
            bZeroFill = FALSE;
...
NORMAL_EXIT:
    ...
    if (IsImmGlbNum (immTypeItm))
    {
        ...
        if (IsImmArb (immTypeItm))
            Myarb_clear (&aplArb10);
        else
            DbgStop ();
    } // End IF

    // We no longer need this storage
    Myarb_clear (&aplArbItm);
    Myf_clear (&aplVfpItm);
    Myq_clear (&aplRatItm);
} // End QFMT_CommonEFIRV
