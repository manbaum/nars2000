...
Stmt:
	...
	|	  LeftOper3 EOL 				{DbgMsgW2 (L"%%Stmt:  EOL LeftOper3");
										 if (lpplLocalVars->bLookAhead)
										 {
											 lpplLocalVars->NameType = NAMETYPE_FN12;
											 YYACCEPT;
...
////										 PushFcnStrand_YY (&$2, 2, TRUE);		 // Dyadic operator (Indirect)
...
Op3Spec:
	...
	| LeftOper3 ASSIGN NameAnyOp1		{DbgMsgW2 (L"%%Op3Spec:  NameAnyOp1" WS_UTF16_LEFTARROW L"LeftOper3");
										 if (lpplLocalVars->bCtrlBreak)
										 {
											 lpplLocalVars->lpYYFcn =
											 MakeFcnStrand_EM_YY (&$1, NAMETYPE_OP1, TRUE);
...
ArrExpr:
	...
	| error   LeftOper3 				{DbgMsgW2 (L"%%ArrExpr:  LeftOper3 error");
										 if (!lpplLocalVars->bLookAhead)
										 {
											 lpplLocalVars->lpYYFcn =
											 MakeFcnStrand_EM_YY (&$2, NAMETYPE_FN12, FALSE);
	...
	| ArrExpr LeftOper3 				{DbgMsgW2 (L"%%ArrExpr:  LeftOper ArrExpr3");
										 if (lpplLocalVars->bCtrlBreak)
										 {
											 lpplLocalVars->lpYYFcn =
											 MakeFcnStrand_EM_YY (&$2, NAMETYPE_FN12, FALSE);
...
Drv1Func:
	...
	|			AmbOp StrandInst		{DbgMsgW2 (L"%%Drv1Func:  StrandInst AmbOp");
										 if (!lpplLocalVars->bLookAhead)
										 {
											 DbgBrk ();

											 // Convert from Operator to Function
											 $1.tkToken.tkFlags.TknType = TKT_FCNIMMED;
											 $1.tkToken.tkFlags.ImmType = IMMTYPE_PRIMFCN;

											 lpplLocalVars->lpYYRes =
											 PushFcnStrand_YY (&$1, 2, TRUE);		 // Monadic function (Indirect)
											 FreeResult (&$1.tkToken);
...
// Ambiguous Left operand function
// Skip Ctrl-Break checking here so the Function Strand processing isn't interrupted
LeftOper3:
					   AmbOp LeftOper	{DbgMsgW2 (L"%%LeftOper3:  LeftOper AmbOp");
										 if (!lpplLocalVars->bLookAhead)
										 {
											 lpplLocalVars->lpYYRes =
											 PushFcnStrand_YY (&$1, 2, TRUE);		 // Monadic operator (Indirect)
	...
	|				   AmbOp AxisFunc	{DbgMsgW2 (L"%%LeftOper3:  AxisFunc AmbOp");
										 if (!lpplLocalVars->bLookAhead)
										 {
											 lpplLocalVars->lpYYRes =
											 PushFcnStrand_YY (&$1, 2, TRUE);		 // Monadic operator (Indirect)
...
DydOp:
	...
	|				   DydOpAxis		{DbgMsgW2 (L"%%DydOp:  DydOpAxis");
										 ...
										 {
											 lpplLocalVars->lpYYRes =
											 PushFcnStrand_YY (&$1, 1, TRUE);		 // Dyadic operator (Indirect)
											 FreeResult (&$1.tkToken);
...
DydOpAxis:
	...
	| '?' ArrExpr '['  DydOp            {DbgMsgW2 (L"%%DydOpAxis:  DydOp[ArrExpr]");
										 ...
										 {
											 lpplLocalVars->lpYYRes =
											 PushFcnStrand_YY (&$4, 2, TRUE);		 // Dyadic operator (Indirect)
											 FreeResult (&$4.tkToken);
