...

{
    ...
    UBOOL        bRet = FALSE,              // TRUE iff result is valid
                 bIsArb = FALSE,            // TRUE iff any var is ARB
...
#undef  lpHeader
    ...
    //                                    T is the storage type (BIFCHNARV...) -- %c
    //                                    N is the NELM -- %d
    //                                    R is the rank -- %d
    //                                    S is the shape -- Rank occurrences of %d
    //                                    val is the series of values of similar form to FMTSTR_GLBCNT
                        ...
                        if (stFlags.Imm)
                        {
                            ...
                            lpaplChar =
                              SavedWsFormGlbVar (lpaplChar,         // Ptr to output save area
                                                ...
                                                &bIsArb,            // Ptr to TRUE iff any var is ARB
    ...
    ProfileSetString (SECTNAME_GENERAL,     // Ptr to the section name
             ...
             bIsArb ? WS_VERSTR             // Ptr to the key value
                    : WS_VERSTR_NOARB,
                      lpDict);              // Ptr to the dictionary
...
////#ifdef DEBUG                                // ***FIXME*** -- Uncomment before ARB release
    // Append the workspace version
    wsprintfW (&lpwszTemp[lstrlenW (lpwszTemp)],
               L" (ver %s)",
       bIsArb ? L"***" WS_VERSTR L"***"
              : WS_VERSTR_NOARB);
////#endif
    // Protect the text
...
#define hGlbSub     (*(LPAPLNESTED *) lpMemObj)
                        ...
                        lpaplChar =
                          SavedWsFormGlbVar (lpaplChar,         // Ptr to output save area
                                             ...
                                             lpSymEntry,        // Ptr to this global's SYMENTRY
                                             lpbIsArb,          // Ptr to TRUE iff any var is ARB
...
////////////////    CheckWsVarLimit (&lpaplChar, lpaplCharStart, &lpSavedWsVar_CB->iIndex, lpDict, lpSavedWsVar_CB);
            ...
            case ARRAY_BA2F:
                ...
                for (uObj = 0; uObj < aplNELMObj; uObj++)
                {
                    // Format the value
                    lpaplChar =
                      FormatAplBA2FFC (lpaplChar,               // Ptr to output save area
                                       lpMemObj,                // The value to format
                                       0,                       // # significant digits (0 = all)
                                       L'.',                    // Char to use as decimal separator
                                       UTF16_BAR,               // Char to use as overbar
    ...
    if (lpHeader NE NULL)
    {
        ...
        *lpaplChar++ = L'(';
         ...
         && !IsEmpty (aplNELMObj))
        {
            ...
            // Calculate the # parts to test
            aplNELMDim = aplNELMObj * TranslateArrayTypeToHCDim (lpHeader->ArrType);

            if (IsAnyVfp (lpHeader->ArrType))
            {
                LPAPLVFP lpaplVfp;
                ...
                *lpuCommPrec = mpfr_get_prec (lpaplVfp++);
                ...
                for (uObj = 1; uObj < aplNELMDim; uObj++)
...
////////////
////////////    for (uObj = 1; uObj < aplNELMDim; uObj++)
////////////    if (*lpuCommPrec NE arb_get_prec (lpaplArb++))
////////////    {
////////////        // Mark as none
////////////        *lpuCommPrec = 0;
////////////
...

{
    ...
    lpaplChar =
      SavedWsFormGlbVar (lpaplChar,                             // Ptr to output save area
                         ...
                         lpSavedWsGlbVarParm->lpbIsArb,         // Ptr to TRUE iff any var is ARB
