...
{ // Name                       Valence       Var?   Std?   Exec Routine          SYS_VARS
    ...
    {WS_UTF16_QUAD L"lr"       , SYSVAR,      TRUE , FALSE, NULL                , SYSVAR_LR      },    // Division Quotient
...
#undef  lpHeader
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_ARB:
            // Attempt to convert the ARB to an integer
            aplInteger = arb_get_sctsx ((LPAPLARB) lpMemRht, &bRet);
...
#undef  lpHeader
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_ARB:
            // Convert the value to a float
            aplFloat = arb_get_d ((LPAPLARB) lpMemRht);
...
#undef  lpHeader
                ...
                // Save the dimension
                *VarArrayBaseToDim (lpMemHdrRes) = aplNELMRht;
...

{
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_ARB:
            ...
            *((LPAPLFLOAT) lpaplLongestRht) = arb_get_d ((LPAPLARB) lpSymGlbRht);
...
//  $ValidNdxLR
//
//  Validate a single value before assigning it to a position in []LR.
//
//  We allow the characters in DEF_QUADLR_ALLOW.
//***************************************************************************
...

{
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_CHAR:
            ...
            return (strchrW (DEF_QUADLR_ALLOW, aplChar) NE NULL);
...

{
    ...
    switch (aplTypeRht)
    {
        ...
        case ARRAY_ARB:
            // Attempt to convert the ARB to an INT
            *lpaplLongestRht = arb_get_sctsx ((LPAPLARB) lpSymGlbRht, &bRet);
...
void ValidPostFPC
    (LPTOKEN lptkNamArg)                    // Ptr to name arg token (may be NULL)
...

{
    ...
    AssignCharScalarCWS (cQuadLR_CWS         , SYSVAR_LR      , lpSymQuad[SYSVAR_LR      ]);    // Division Quotient
...

{
    ...
    lphtsDst->lpSymQuad[SYSVAR_LR      ]->stData.stLongest = lphtsSrc->lpSymQuad[SYSVAR_LR      ]->stData.stLongest;
...

{
    ...
    aSysVarValidSet[SYSVAR_LR      ] = ValidSetLR_EM       ;
    ...
    aSysVarValidPost[SYSVAR_LR      ] = ValidPostNone       ;
    ...
    aSysVarValidPost[SYSVAR_FPC     ] = ValidPostFPC        ;
