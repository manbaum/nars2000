    ...
    if (IsSimpleGlbNum (aplTypeRht))
    {
        ...
        if (lpPrimIdent->IsBool)
        {
            ...
            switch (aplTypeRes)
            {
                ...
                case ARRAY_ARB:
                    for (uRht = 0; uRht < aplNELMRes; uRht++)
                        arb_init_set_sx (((LPAPLARB) lpMemRes)++, lpPrimIdent->bIdentElem);
                    break;
                ...
                case ARRAY_BA2F:
                    for (uRht = 0; uRht < aplNELMRes; uRht++)
                        arb2f_init_set_sx (((LPAPLBA2F) lpMemRes)++, lpPrimIdent->bIdentElem);
                    break;

                case ARRAY_BA4F:
                    for (uRht = 0; uRht < aplNELMRes; uRht++)
                        arb4f_init_set_sx (((LPAPLBA4F) lpMemRes)++, lpPrimIdent->bIdentElem);
                    break;

                case ARRAY_BA8F:
                    for (uRht = 0; uRht < aplNELMRes; uRht++)
                        arb8f_init_set_sx (((LPAPLBA8F) lpMemRes)++, lpPrimIdent->bIdentElem);
                    break;
        ...
        } else
        {
            ...
            switch (aplTypeRes)
            {
                ...
                case ARRAY_ARB:
                    for (uRht = 0; uRht < aplNELMRes; uRht++)
                        arb_init_set_d (((LPAPLARB) lpMemRes)++, lpPrimIdent->fIdentElem);
                    break;
                ...
                switch (ExceptionCode)
                {
                    ...
                    case EXCEPTION_RESULT_BA8F:
                        ...
                        if (IsSimpleNum (aplTypeRes))
                        {
                            ...
                            // It's now a HC[1248][FRVB] result
...

{
    ...
    HGLOBAL           hGlbRes = NULL,       // Result global memory handle
                      ...
                      hGlbSub;              // Subarray ...
    APLSTYPE          aplTypeRht,           // Right arg storage type
                      aplTypeRht2,          // Right arg base storage type
...
#undef  lpHeader
    ...
    if (bRad0Only)
    {
        ...
        APLNELM  aplNELMHCRht,      // Loop limit
                 uRht;              // Loop counter
        LPAPLARB lpMemRht;          // Ptr to left arg global memory data
...
RESTART_EXCEPTION:
    ...
    if (bRad0Only)
    {
        // The left arg is Nested
        // The right arg is a singleton

        int      iHCDimRht;         // HC Dimension (1, 2, 4, 8)
        int      i;                 // Loop counter

        Assert (IsAnyArb (aplTypeRht));

        // Calculate the HC Dimension (1, 2, 4, 8)
        iHCDimRht = TranslateArrayTypeToHCDim (aplTypeRht);

        // Loop through the parts of the right arg
        for (i = 0; i < iHCDimRht; i++)
        // If the radius is non-zero, ...
        if (!mag_is_zero (arb_radref (&lpatRht->aplBA8F.parts[i])))
            goto RIGHT_DOMAIN_EXIT;
...
#undef  lpAPA
    ...
    for (uRes = 0; uRes < (APLNELMSIGN) aplNELMLft; uRes++)
    {
        ...
        // The left arg is hetero, simple numeric, or simple global numeric
        {
            ...
            {
                // Calculate the left & right HC Dimensions
                int iHCDimLft = TranslateArrayTypeToHCDim (aplTypeHetLft),
                    iHCDimRht = TranslateArrayTypeToHCDim (aplTypeRht);

                // Set the Left & Right HC Dimension indices before promotion
                lpPrimSpec->iHCDimIndexLft = aHCDimToIndex[iHCDimLft];
                lpPrimSpec->iHCDimIndexRht = aHCDimToIndex[iHCDimRht];
...
RESTART_EXCEPTION:
    ...
    if (bRad0Only)
    {
        // The left arg is a singleton
        // The right arg is nested

        int      iHCDimLft;         // HC Dimension (1, 2, 4, 8)
        int      i;                 // Loop counter

        Assert (IsAnyArb (aplTypeLft));

        // Calculate the HC Dimension (1, 2, 4, 8)
        iHCDimLft = TranslateArrayTypeToHCDim (aplTypeLft);

        // Loop through the parts of the left arg
        for (i = 0; i < iHCDimLft; i++)
        // If the radius is non-zero, ...
        if (!mag_is_zero (arb_radref (&lpatLft->aplBA8F.parts[i])))
            goto LEFT_DOMAIN_EXIT;
...
#undef  lpAPA
    ...
    for (uRes = 0; uRes < (APLNELMSIGN) aplNELMRht; uRes++)
    {
        ...
        // The right arg is hetero or simple global numeric
        {
            ...
            {
                // Calculate the left & right HC Dimensions
                int iHCDimLft = TranslateArrayTypeToHCDim (aplTypeLft),
                    iHCDimRht = TranslateArrayTypeToHCDim (aplTypeHetRht);
...
RESTART_EXCEPTION:
        ...
        __try
        {
                 ...
                 uRes++, uLft += uLftInc, uRht += uRhtInc)
            {
                ...
                {
                    // Calculate the left & right HC Dimensions for comparison purposes
                    int iHCDimLft = TranslateArrayTypeToHCDim (aplTypeLft),
                        iHCDimRht = TranslateArrayTypeToHCDim (aplTypeRht),
                        iHCDimRes = TranslateArrayTypeToHCDim (aplTypeRes);

                    // If the dimensions of the argument and result differ,
                    //   promote the arg to the type of the result
                    // This can happen when (say) a VFP/ARB blows up to HC2V/BA2F (e.g., log -2.5v  or  (-4)o-0.5v)
                    if ((iHCDimLft < iHCDimRes)
                     || (iHCDimRht < iHCDimRes))
                        aplTypeCom = aplTypeRes;
                    else
                        // Get the common storage type between the left & right args
                    ...
                    // Calculate the left & right HC Dimensions for calculation purposes
                    iHCDimLft = TranslateArrayTypeToHCDim (aplTypeHetLft),
                    iHCDimRht = TranslateArrayTypeToHCDim (aplTypeHetRht),
                    ...
                    // Special case ARB
                    if (IsAnyArb (aplTypeRes))
                        // Set as common storage type as we don't support
                        //   combinations such as ARBisBvB, etc.
                        aplTypeCom = aplTypeRes;
...

{
    ...
    // If the result is any Arb, ...
    if (IsAnyArb (aplTypeRes))
    {
        // If the result is ARB, ...
        if (IsRealArb (aplTypeRes))
            return (&lpPrimSpec->ARBisBA1F )[aArrayTypeToHCDimIndex[aplTypeRht]];
        else
            return (&lpPrimSpec->BA1FisBA1F)[aArrayTypeToHCDimIndex[aplTypeRht]];
    } else
    {
    ...
    switch (aplTypeRes)
    {
        ...
        case ARRAY_BOOL:        // Result is Boolean
            ...
            switch (aplTypeCom)
            {
                ...
                case ARRAY_ARB:
                case ARRAY_BA2F:        // N.B.:  We're assuming that the types in [ARB, BA8F] are contiguous
                case ARRAY_BA4F:
                case ARRAY_BA8F:
                    return (&lpPrimSpec->BisBA1FvBA1F)[aArrayTypeToHCDimIndex[aplTypeCom]];
        ...
        case ARRAY_BA4F:
        case ARRAY_BA8F:
            return (&lpPrimSpec->BA1FisBA1FvBA1F)[aArrayTypeToHCDimIndex[aplTypeRes]];
